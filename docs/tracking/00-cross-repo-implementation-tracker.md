...
The plugin inspect failed-at timestamp projection slice is checkpointed in
`b3bf64a5`. Any follow-up changes should target the next queue head only:
...
| OZ-PLUGIN-001 | Real installed plugin module import/activation | Plugin inspect failed-at timestamp projection checkpointed in `b3bf64a5` | Repo-wide +0.1%, CLI/runtime +0.1% | Continue real installed module import/activation depth |
