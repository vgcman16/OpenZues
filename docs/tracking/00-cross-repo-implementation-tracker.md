...
| Active gateway/session/tool-contract path | ~99.966% | Near-complete bounded local path | `docs/openclaw-parity-progress.md` |
...
| OZ-RT-001 | Runtime-control hard gaps | Artifact transcript gateway methods checkpointed in `13eddac7`; `update.status` checkpointed in `59a36693`; `diagnostics.stability` checkpointed in `0a4deddc`; base method sweep checkpointed in `8a0e6ac6` | Repo-wide +0.1%, active gateway/method +0.1% | Continue native-hook, web-push, and memory doctor method gaps |
...
gateway/session/tool-contract parity moves to ~99.966%. Continue native-hook,
