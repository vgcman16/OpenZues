...
`b56d15d7`; the discord facade is checkpointed in `307777d8`, the compat
facade is checkpointed in `f21a22bd`, the channel-runtime facade is
checkpointed in `1cc947f3`, the setup facade is checkpointed in `ec94f934`,
and the exact `config-types` type-only barrel was reverified on 2026-05-07.
...
breadth.
Provider addendum: `OZ-PROV-001GE` Feishu media implicit reply fanout is
source/test checkpointed in `ae7e9ed1`, moving provider-native inbound/outbound
breadth to ~99.999981%. Continue Discord audio-as-voice depth, remaining
provider-specific media/reply edges, deeper runtime/plugin activation, or
companion breadth.
Provider addendum: `OZ-PROV-001GH` Signal receive routing is source/test
checkpointed in `25279aa3`, moving provider-native inbound/outbound breadth
to ~99.999984%. Continue remaining provider-specific media/reply edges, deeper
installed plugin activation, packaging/distribution, or companion breadth.
...
installed plugin activation, ACP bridge lifecycle depth, or companion breadth.
Runtime addendum: `OZ-PKG-001DB` package-update post-core fresh-process
handoff is source/test checkpointed in `81f715ab`, moving runtime/CLI/doctor
native-bridge parity to ~99.999992%. Continue deeper installed plugin
activation, ACP bridge lifecycle depth, provider media edges, or companion
breadth.
Runtime addendum: `OZ-PKG-001DC` runtime exit-signal labeling is source/test
checkpointed in `37af5b93`, moving runtime/CLI/doctor native-bridge parity to
~99.999993%. Continue deeper installed plugin activation, ACP bridge lifecycle
depth, provider media edges, packaging/distribution, or companion breadth.
Plugin addendum: `OZ-PLUGIN-001ZZA` installed facade registry fallback is
...
browser/canvas/nodes/voice bounded-command parity to ~99.9%.
`OZ-CANVAS-001O` status/doctor route breadth is source/test checkpointed in
`4b1f8028`, moving active gateway/session/tool parity to ~99.956% and
browser/canvas/nodes/voice bounded-command parity to ~99.92%.
`OZ-CANVAS-001P` snapshot artifact route breadth is source/test checkpointed
in `2ecdba40`, moving active gateway/session/tool parity to ~99.957% and
browser/canvas/nodes/voice bounded-command parity to ~99.94%.
`OZ-CANVAS-001Q` response-body route breadth is source/test checkpointed in
`e7084f1f`, moving active gateway/session/tool parity to ~99.958% and
browser/canvas/nodes/voice bounded-command parity to ~99.95%. Continue
provider-specific media/reply edges, companion breadth, or the next packaging
edge.
`OZ-CANVAS-001R` dialog-hook route breadth is source/test checkpointed in
`4be4d540`, moving active gateway/session/tool parity to ~99.959% and
browser/canvas/nodes/voice bounded-command parity to ~99.96%. Continue
profile mutation, permissions, locale/timezone, label-action route breadth,
provider-specific media/reply edges, companion breadth, or the next packaging
edge.
`OZ-CANVAS-001S` locale/timezone route breadth is source/test checkpointed in
`003b0d5a`, moving active gateway/session/tool parity to ~99.960% and
browser/canvas/nodes/voice bounded-command parity to ~99.97%. Continue
profile mutation, permissions, label-action route breadth, provider-specific
media/reply edges, companion breadth, or the next packaging edge.
`OZ-CANVAS-001T` tab label action route breadth is source/test checkpointed in
`c6fb16d3`, moving active gateway/session/tool parity to ~99.961% and
browser/canvas/nodes/voice bounded-command parity to ~99.98%. Continue
profile mutation boundary review, permissions, provider-specific media/reply
edges, companion breadth, or the next packaging edge.
`OZ-CANVAS-001U` permission grant route breadth is source/test checkpointed in
`1a27af54`, moving active gateway/session/tool parity to ~99.962% and
browser/canvas/nodes/voice bounded-command parity to ~99.99%. Continue
persistent profile boundary review, provider-specific media/reply edges,
companion breadth, or the next packaging edge.
`OZ-CANVAS-001V` persistent browser profile mutation guard review is verified
against `extensions/browser/src/browser/request-policy.ts`, moving active
gateway/session/tool parity to ~99.963% and browser/canvas/nodes/voice
bounded-command parity to ~99.995%. Browser local-request route breadth has no
remaining OpenClaw-backed queue head in the accepted native adapter scope.
...
breadth, or the next package startup/update edge.
Companion addendum: `OZ-COMP-001P` device CLI mutation commands
`remove`, `clear`, `reject`, `rotate`, and `revoke` are source/test
checkpointed in `1c25fdbb`. Packaging/companion app parity moves to ~5.3%;
continue remote device CLI fallback/auth ergonomics, provider/runtime breadth,
or the next package startup/update edge.
Companion addendum: `OZ-COMP-001Q` device approve preview gateway flags are
source/test checkpointed in `204bcdd2`. `--url` and `--timeout` survive in the
safe rerun command, while `--token` and `--password` become
`requiresAuthFlags` booleans without leaking secrets. Packaging/companion app
parity moves to ~5.4%; continue actual remote device CLI fallback/auth
dispatch, provider/runtime breadth, or the next package startup/update edge.
Companion addendum: `OZ-COMP-001R` remote device list/approve dispatch is
source/test checkpointed in `fa6d230a`. Explicit `--url` on `devices list` and
`devices approve <requestId>` now uses the fakeable remote gateway method
bridge with `--token`, `--password`, and `--timeout` forwarding.
Packaging/companion app parity moves to ~5.5%; continue implicit/default
configured remote fallback, device mutation remote flags, provider/runtime
breadth, or the next package startup/update edge.
Companion addendum: `OZ-COMP-001S` remote device mutation dispatch is
source/test checkpointed in `c54866d9`. `devices remove`, `clear`, `reject`,
`rotate`, and `revoke` now forward explicit remote gateway/auth/timeout flags
through the same bridge. Packaging/companion app parity moves to ~5.6%;
continue implicit/default configured remote fallback, provider/runtime breadth,
or the next package startup/update edge.
Companion addendum: `OZ-COMP-001T` configured remote device CLI defaults are
source/test checkpointed in `aa2cbf50`. Device commands now use saved
`gateway.remote.url` and remote auth when explicit URL/auth flags are omitted.
Packaging/companion app parity moves to ~5.7%; continue loopback
pairing-required local fallback, provider/runtime breadth, or the next package
startup/update edge.
Companion addendum: `OZ-COMP-001U` loopback device local fallback is
source/test checkpointed in `3616f48b`. Configured loopback remotes now fall
back to the local pairing service on pairing-required or pending-approval
failures, while explicit `--url` remains remote-strict. Packaging/companion
app parity moves to ~5.8%; continue provider/runtime breadth, packaging/
distribution, or the next package startup/update edge.
Companion addendum: `OZ-COMP-001V` device approval-state preview metadata is
source/test checkpointed in `a45702fc`. `devices approve --latest --json` now
reports OpenClaw-style requested/approved access summaries and approval kind.
Packaging/companion app parity moves to ~5.9%; continue provider/runtime
breadth, packaging/distribution, or the next package startup/update edge.
...
edge.
Provider addendum: `OZ-PROV-001FL` authenticated Zalo webhook ingress is
source/test checkpointed in `c0e8588e`; provider-native inbound/outbound
breadth moves to ~99.9992%. Continue Zalo webhook session delivery/replay/media
breadth, provider-specific media/reply edges, deeper installed plugin
activation, companion breadth, or the next packaging edge.
Provider addendum: `OZ-PROV-001FM` Zalo text webhook session delivery and
message-id replay dedupe is source/test checkpointed in `edcccea3`;
provider-native inbound/outbound breadth moves to ~99.9993%. Continue Zalo
image/media webhook breadth, provider-specific media/reply edges, deeper
installed plugin activation, companion breadth, or the next packaging edge.
Provider addendum: `OZ-PROV-001FN` Zalo image webhook media URL delivery is
source/test checkpointed in `6c87d9e5`; provider-native inbound/outbound
breadth moves to ~99.9994%. Continue Zalo remote-media download/staging depth,
provider-specific media/reply edges, deeper installed plugin activation,
companion breadth, or the next packaging edge.
Provider addendum: `OZ-PROV-001FO` fakeable Zalo inbound image media staging is
source/test checkpointed in `652f0938`; provider-native inbound/outbound
breadth moves to ~99.9995%. Continue production Zalo remote-media fetch wiring,
provider-specific media/reply edges, deeper installed plugin activation,
companion breadth, or the next packaging edge.
Provider addendum: `OZ-PROV-001FP` production Zalo inbound media fetch is
source/test checkpointed in `00241ee2`; provider-native inbound/outbound
breadth moves to ~99.9996%. Continue Zalo inbound authorization/group-policy
breadth, provider-specific media/reply edges, deeper installed plugin
activation, companion breadth, or the next packaging edge.
Provider addendum: `OZ-PROV-001FQ` Zalo direct-DM disabled policy is
source/test checkpointed in `ac0e17e3`; provider-native inbound/outbound
breadth moves to ~99.9997%. Continue Zalo allowlist and group-policy
authorization breadth, provider-specific media/reply edges, deeper installed
plugin activation, companion breadth, or the next packaging edge.
Provider addendum: `OZ-PROV-001FR` Zalo group allowlist policy is source/test
checkpointed in `d0d548e2`; provider-native inbound/outbound breadth moves to
~99.9998%. Continue remaining Zalo allowlist/pairing breadth,
provider-specific media/reply edges, deeper installed plugin activation,
companion breadth, or the next packaging edge.
Provider addendum: `OZ-PROV-001FS` Zalo direct-DM pairing challenge is
source/test checkpointed in `74ce8fbd`; provider-native inbound/outbound
breadth moves to ~99.99985%. Continue remaining Zalo pairing-store
approval/allowFrom restoration breadth, provider-specific media/reply edges,
deeper installed plugin activation, companion breadth, or the next packaging
edge.
Provider addendum: `OZ-PROV-001FT` Zalo pairing allowFrom-store authorization
is source/test checkpointed in `1b2d5009`; provider-native inbound/outbound
breadth moves to ~99.99987%. Continue remaining Zalo pairing approval/allowFrom
mutation breadth, provider-specific media/reply edges, deeper installed plugin
activation, companion breadth, or the next packaging edge.
Provider addendum: `OZ-PROV-001FU` Zalo pairing approval store mutation is
source/test checkpointed in `9409ad9b`; provider-native inbound/outbound
breadth moves to ~99.99989%. Continue native pairing list/approve CLI/RPC
surface, provider-specific media/reply edges, deeper installed plugin
activation, companion breadth, or the next packaging edge.
Provider addendum: `OZ-PROV-001FV` Zalo pairing request listing is source/test
checkpointed in `d1c79fea`; provider-native inbound/outbound breadth moves to
~99.99990%. Continue native pairing list/approve CLI/RPC surface,
provider-specific media/reply edges, deeper installed plugin activation,
companion breadth, or the next packaging edge.
Provider/CLI addendum: `OZ-PROV-001FW` Zalo pairing CLI list/approve is
source/test checkpointed in `6de9e5a1`; provider-native inbound/outbound
breadth moves to ~99.99991%, and runtime/CLI/doctor plus CLI/operator parity
move to ~99.99993%. Continue provider-specific media/reply edges, deeper
installed plugin activation, companion breadth, or the next packaging edge.
Provider/CLI addendum: `OZ-PROV-001FX` Zalo pairing approval notification CLI
is source/test checkpointed in `3a77ccf5`; provider-native inbound/outbound
...
installed plugin activation, companion breadth, or the next packaging edge.
Provider/CLI addendum: `OZ-PROV-001FY` Zalo pairing command-owner bootstrap
is source/test checkpointed in `c105b1c5`; provider-native inbound/outbound
breadth moves to ~99.99993%, and runtime/CLI/doctor plus CLI/operator parity
move to ~99.99995%. Continue provider-specific media/reply edges, deeper
installed plugin activation, companion breadth, or the next packaging edge.
Provider/CLI addendum: `OZ-PROV-001FZ` Zalo pairing list default is
source/test checkpointed in `0b791e74`; provider-native inbound/outbound
breadth moves to ~99.99994%, and runtime/CLI/doctor plus CLI/operator parity
move to ~99.99996%. Continue provider-specific media/reply edges, deeper
installed plugin activation, companion breadth, or the next packaging edge.
Provider/CLI addendum: `OZ-PROV-001GA` Zalo pairing command-owner bootstrap
explanation is source/test checkpointed in `646fa3d0`; provider-native
inbound/outbound breadth moves to ~99.99995%, and runtime/CLI/doctor plus
CLI/operator parity move to ~99.99997%. Continue provider-specific
media/reply edges, deeper installed plugin activation, companion breadth, or
the next packaging edge.
Provider/CLI addendum: `OZ-PROV-001GB` Zalo pairing approval not-found text
is source/test checkpointed in `433368f1`; provider-native inbound/outbound
breadth moves to ~99.99996%, and runtime/CLI/doctor plus CLI/operator parity
move to ~99.99998%. Continue provider-specific media/reply edges, deeper
installed plugin activation, companion breadth, or the next packaging edge.
Provider/CLI addendum: `OZ-PROV-001GC` disabled channel capability actions are
source/test checkpointed in `0b5231cc`; provider-native inbound/outbound
breadth moves to ~99.99997%, and runtime/CLI/doctor plus CLI/operator parity
move to ~99.99999%. Continue provider-specific media/reply edges, deeper
installed plugin activation, companion breadth, or the next packaging edge.
...
parity moves to ~99.965%. `diagnostics.stability` is the next gateway method
head.
Runtime/gateway addendum: `OZ-RT-001BD` `diagnostics.stability` payload-free
snapshot is source/test checkpointed in `0a4deddc`; active
gateway/session/tool-contract parity moves to ~99.966%. `doctor.memory.remHarness`
is the next gateway method head.
Runtime/gateway addendum: `OZ-RT-001BE` `doctor.memory.remHarness` read-only
REM preview is source/test checkpointed in `5e6d43f6`; active
gateway/session/tool-contract parity moves to ~99.967%. `nativeHook.invoke` is
the next gateway method head.
Runtime/gateway addendum: `OZ-RT-001BF` `nativeHook.invoke` codex relay is
source/test checkpointed in `587c181b`; active gateway/session/tool-contract
parity moves to ~99.968%. The next queue head at that checkpoint was the
web-push helper method cluster.
Runtime/gateway addendum: `OZ-RT-001BG` `push.web.vapidPublicKey`,
`push.web.subscribe`, `push.web.unsubscribe`, and `push.web.test` are
source/test checkpointed in `af0fc977`; active gateway/session/tool-contract
parity moves to ~99.969%. The OpenClaw gateway `server-methods/*.ts` method
name sweep currently has no missing local gateway owner names. Continue
...
CLI addendum: `OZ-CLI-001A` `gateway status --deep` native command alias is
source/test checkpointed in `005dc599`; runtime/CLI/doctor and CLI/operator
parity move to ~99.99991%. Continue remaining packaging/distribution breadth,
provider-specific media/reply edges, deeper installed plugin activation, or
companion breadth.
Packaging addendum: `OZ-PKG-001CZ` restart-health follow-up diagnostics are
source/test checkpointed in `8e989b2d`; runtime/CLI/doctor and CLI/operator
parity move to ~99.99992%. Continue remaining packaging/distribution breadth,
provider-specific media/reply edges, deeper installed plugin activation, or
companion breadth.
...
- [x] `OZ-RT-001BB` Gateway transcript artifact methods
  ...
  - Target: `src/openzues/services/gateway_node_methods.py`,
    `src/openzues/services/gateway_method_policy.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: `artifacts.list`, `artifacts.get`, and `artifacts.download`
    accept OpenClaw-shaped `sessionKey` / `runId` / `taskId` queries, discover
    image/audio/file content blocks from session transcripts, expose stable
    artifact summaries without raw bytes, and return base64/url download
    payloads or typed artifact errors.
  - Evidence required: focused artifact gateway test, adjacent sessions/artifact
    gateway test, policy test, ruff, mypy
  - Status: checkpointed in `13eddac7`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green
    `python -m pytest tests\test_gateway_node_methods.py::test_artifacts_methods_discover_and_download_inline_session_artifacts -q`
    (`unsupported method: artifacts.list` before implementation, then `1
...
- [x] `OZ-RT-001BD` Gateway diagnostics stability snapshot
  ...
  - Target: `src/openzues/services/gateway_diagnostics.py`,
    ...
    `tests/test_gateway_node_methods.py`,
    `tests/test_gateway_method_policy.py`
  - Contract: `diagnostics.stability` returns a payload-free stability snapshot
    with capacity, count/dropped/seq metadata, sanitized event records,
    `limit` / `type` / `sinceSeq` filtering, and `byType` / `payload.large`
    summaries while rejecting invalid limits.
  - Evidence required: focused diagnostics-stability gateway tests, adjacent
    logs/diagnostics gateway tests, policy test, ruff, mypy
  ...
  - Last verified: 2026-05-12, focused red/green
    ...
    (`ModuleNotFoundError` before implementation, then `2 passed`), adjacent
    gateway proof
...
- [x] `OZ-RT-001BE` Gateway memory REM-harness preview
  ...
  - Target: `src/openzues/services/gateway_node_methods.py`,
    ...
    `tests/test_gateway_node_methods.py`,
    `tests/test_gateway_method_policy.py`
  - Contract: `doctor.memory.remHarness` accepts `grounded`,
    `includePromoted`, and clamped `limit`, scans workspace memory markdown,
    and returns OpenClaw-shaped `remConfig`, `deepConfig`, `rem`, `grounded`,
    and `deep` preview payloads without mutating memory files.
  - Evidence required: focused REM-harness gateway test, adjacent memory-doctor
    tests, policy test, ruff, mypy
  ...
  - Last verified: 2026-05-12, focused red/green
    ...
    (`unsupported method: doctor.memory.remHarness` before implementation,
    then `1 passed`), adjacent memory-doctor proof
...
- [x] `OZ-RT-001BF` Gateway native hook relay invocation
  ...
  - Target: `src/openzues/services/gateway_native_hook_relay.py`,
    ...
    `tests/test_gateway_node_methods.py`,
    `tests/test_gateway_method_policy.py`
  - Contract: `nativeHook.invoke` validates codex provider/event/relay ids,
    invokes registered live relays, records normalized invocation metadata,
    returns Codex no-op process responses, and projects missing relay ids as
    OpenClaw-shaped invalid requests.
  - Evidence required: focused native-hook gateway tests, adjacent gateway
    tests, admin-scope policy test, ruff, mypy
  ...
  - Last verified: 2026-05-12, focused red/green
    ...
    (`2 passed, 17 deselected`), `ruff check`, `mypy`, and focused
    `git diff --check`.

- [x] `OZ-RT-001BG` Gateway web-push methods
  - Source: `openclaw-main/src/gateway/server-methods/push.ts`,
    `openclaw-main/src/infra/push-web.ts`,
    `openclaw-main/src/gateway/protocol/schema/push.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_web_push.py`,
    `src/openzues/services/gateway_node_methods.py`,
    `src/openzues/services/gateway_method_policy.py`,
    `tests/test_gateway_node_methods.py`,
    `tests/test_gateway_method_policy.py`
  - Contract: `push.web.vapidPublicKey`, `push.web.subscribe`,
    `push.web.unsubscribe`, and `push.web.test` provide env-first/persisted
    VAPID public keys, endpoint-hashed subscription upsert/removal, fakeable
    broadcast test delivery, expired endpoint cleanup, and OpenClaw-shaped
    invalid/unavailable errors.
  - Evidence required: focused web-push gateway tests, adjacent push gateway
    tests, method-scope policy test, OpenClaw handler-name sweep, ruff, mypy
  - Status: checkpointed in `af0fc977`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green
    ...
    (`2 passed, 17 deselected`), handler-string sweep with no missing OpenClaw
    `server-methods/*.ts` method names, `ruff check`, `mypy`, and focused
    `git diff --check`.

- [x] `OZ-PROV-001FM` Zalo text webhook session delivery and replay dedupe
  ...
  - Contract: Zalo `message.text.received` webhook payloads route to native
    ...
    metadata, and account/event/chat/sender/message-id replay dedupe before
    dispatch.
  - Evidence required: focused Zalo webhook session-delivery/replay proof,
    adjacent Zalo webhook/provider proof, ruff, mypy
...
- [x] `OZ-PROV-001FN` Zalo image webhook media URL delivery
  ...
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_ops_mesh.py`
  - Contract: Zalo `message.image.received` webhook payloads route to native
    session delivery using caption text or `<media:image>`, preserve
    `photo_url` as delivery media metadata, and continue using message-id
    replay dedupe before dispatch.
...
- [x] `OZ-PROV-001FO` fakeable Zalo inbound image media staging
  - Source: `openclaw-main/extensions/zalo/src/monitor.ts`,
    `openclaw-main/extensions/zalo/src/test-support/lifecycle-test-support.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_ops_mesh.py`
  - Contract: Zalo image webhook media can be fetched through a fakeable
    native adapter, stored in OpenZues' inbound attachment store, and projected
    as `MediaPath`, `MediaType`, `MediaUrls`, and `stagedMedia` metadata while
    preserving the original `photo_url`.
...
- [x] `OZ-PROV-001FP` production Zalo inbound media fetch
  - Source: `openclaw-main/extensions/zalo/src/monitor.ts`,
    `openclaw-main/extensions/zalo/src/api.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_ops_mesh.py`
  - Contract: when native app state storage is available, Zalo image webhook
    staging can fetch `photo_url` without an injected adapter, enforce the
    upstream 5 MB cap, preserve content type, and save bytes through the shared
    inbound attachment store.
  - Evidence required: focused production Zalo media fetch proof, adjacent
...
- [x] `OZ-PROV-001FQ` Zalo direct-DM disabled policy
  - Source: `openclaw-main/extensions/zalo/src/monitor.ts`,
    `openclaw-main/extensions/zalo/src/config-schema.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_ops_mesh.py`
  - Contract: configured `channels.zalo.accounts.<id>.dmPolicy="disabled"`
    skips direct Zalo webhook delivery before session dispatch and returns
    explicit skip metadata without invoking the session delivery service.
...
- [x] `OZ-PROV-001FR` Zalo group allowlist policy
  - Source: `openclaw-main/extensions/zalo/src/monitor.ts`,
    `openclaw-main/extensions/zalo/src/group-access.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_ops_mesh.py`
  - Contract: configured `groupPolicy="allowlist"` and `groupAllowFrom`
    skip non-allowlisted Zalo group senders before session dispatch and return
    explicit skip metadata.
...
- [x] `OZ-PROV-001FS` Zalo direct-DM pairing challenge
  - Source: `openclaw-main/extensions/zalo/src/monitor.ts`,
    `openclaw-main/src/pairing/pairing-challenge.ts`,
    `openclaw-main/src/pairing/pairing-store.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_ops_mesh.py`
  - Contract: configured `dmPolicy="pairing"` skips unknown direct senders,
    creates a scoped Zalo pairing request, sends a pairing reply when outbound
    runtime is available, and returns explicit skip/pairing metadata.
...
- [x] `OZ-PROV-001FT` Zalo pairing allowFrom-store authorization
  - Source: `openclaw-main/extensions/zalo/src/monitor.ts`,
    `openclaw-main/src/plugin-sdk/command-auth.ts`,
    `openclaw-main/src/pairing/allow-from-store-file.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_ops_mesh.py`
  - Contract: `dmPolicy="pairing"` reads the account-scoped Zalo pairing
    `allowFrom` store and delivers already-approved direct senders without
    issuing another pairing challenge.
...
- [x] `OZ-PROV-001FU` Zalo pairing approval store mutation
  - Source: `openclaw-main/src/pairing/pairing-store.ts`,
    `openclaw-main/src/cli/pairing-cli.ts`,
    `openclaw-main/src/pairing/allow-from-store-file.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_ops_mesh.py`
  - Contract: approving a pending Zalo pairing code removes it from the
    account-scoped pending store, prunes expired requests, adds the sender to
    `zalo-<account>-allowFrom.json`, and returns OpenClaw-shaped success or
    precise storage/not-found error metadata.
...
- [x] `OZ-PROV-001FV` Zalo pairing request listing
  - Source: `openclaw-main/src/pairing/pairing-store.ts`,
...
- [x] `OZ-PROV-001FW` Zalo pairing CLI list/approve
...
- [x] `OZ-PROV-001FX` Zalo pairing approval notification CLI
  - Source: `openclaw-main/src/cli/pairing-cli.ts`,
...
- [x] `OZ-PROV-001FY` Zalo pairing command-owner bootstrap
  - Source: `openclaw-main/src/cli/pairing-cli.ts`,
    `openclaw-main/src/commands/doctor-command-owner.ts`,
    `openclaw-main/src/config/config.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: after successful Zalo pairing approval, the CLI writes
    `zalo:<sender>` into `commands.ownerAllowFrom` only when no command owners
    are configured and reports bootstrap metadata.
...
- [x] `OZ-PROV-001FZ` Zalo pairing list default
  - Source: `openclaw-main/src/cli/pairing-cli.ts`,
    `openclaw-main/src/cli/pairing-cli.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues pairing list` uses Zalo as the sole native pairing
    channel when no positional channel or `--channel` is provided, while
    preserving explicit channel/account/JSON behavior.
...
- [x] `OZ-PROV-001GA` Zalo pairing command-owner explanation
  - Source: `openclaw-main/src/cli/pairing-cli.ts`,
    `openclaw-main/src/cli/pairing-cli.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: when `openzues pairing approve` bootstraps command ownership,
    human output explains that `commands.ownerAllowFrom` was empty.
...
- [x] `OZ-PROV-001GB` Zalo pairing approval not-found text
  - Source: `openclaw-main/src/cli/pairing-cli.ts`,
    `openclaw-main/src/cli/pairing-cli.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: when `openzues pairing approve` cannot find a pending request
    for a code, human output says `No pending pairing request found for code:
    <code>` instead of exposing the native machine reason.
...
- [x] `OZ-PROV-001GC` Disabled channel capability actions
  - Source: `openclaw-main/extensions/zalo/src/actions.ts`,
    `openclaw-main/extensions/zalo/src/actions.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: channel capability discovery does not advertise message actions
    for disabled accounts/routes, matching Zalo `describeMessageTool` returning
    `null` for disabled selected accounts.
  ...
  - Last verified: 2026-05-12, focused red/green
    ...
    (disabled Zalo account still advertised `send`/`broadcast` before
    implementation, then `1 passed`), adjacent CLI proof
...
- [x] `OZ-PROV-001GE` Feishu/Lark multi-media implicit reply fanout
  - Source: `openclaw-main/src/infra/outbound/reply-policy.ts`,
    `openclaw-main/src/infra/outbound/message-plan.ts`,
    `openclaw-main/src/infra/outbound/message-plan.test.ts`
  - References: Hermes/Warp `none`
...
- [x] `OZ-PROV-001DD` Tlon image-media upload hook
  - Source: `openclaw-main/extensions/tlon/src/channel.runtime.ts`,
  ...
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: route-backed Tlon media sends keep caption text separate from
    generated media summaries, fetch image URLs through a fakeable upload hook,
    build Tlon stories with uploaded image URLs, persist uploaded `mediaUrls`,
    and fall back to the original URL when upload fails.
  - Evidence required: focused Tlon image-media send test, focused image
    fetch/upload helper test, adjacent native-provider proof, ruff, mypy
  - Status: checkpointed in `0c18844d`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green media proof (`1 failed`
    before implementation, then `1 passed`), helper proof (`2 passed`),
    adjacent native-provider proof (`10 passed, 376 deselected`), `ruff check
    src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, and `mypy
    src\openzues\services\ops_mesh.py`.

- [x] `OZ-PROV-001DE` Tlon hosted Memex media upload
  - Source: `openclaw-main/extensions/tlon/src/tlon-api.ts`,
    `openclaw-main/extensions/tlon/src/tlon-api.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: hosted Tlon media uploads authenticate, scry storage
    configuration, credentials, and genuine secret, request a trusted Memex
    upload URL, reject untrusted upload URLs, PUT bytes with cache/content-type
    headers, and return the trusted hosted URL.
  - Evidence required: focused hosted Memex upload test, trusted-domain guard
    test, adjacent native-provider proof, ruff, mypy
  - Status: checkpointed in `f742ba8a`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green Memex proof (`1 failed`
    before implementation, then `1 passed`), security proof (`2 passed`),
    adjacent native-provider proof (`12 passed, 376 deselected`), `ruff check
    src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, and `mypy
    src\openzues\services\ops_mesh.py`.

- [x] `OZ-PROV-001DF` Tlon custom S3 media upload
  - Source: `openclaw-main/extensions/tlon/src/tlon-api.ts`,
    `openclaw-main/extensions/tlon/src/tlon-api.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: when Tlon storage credentials are configured, OpenZues resolves
    bucket/public URL/region, presigns a path-style S3-compatible PUT, uploads
    bytes, and returns the configured public URL.
  - Evidence required: focused custom S3 credential upload test, adjacent
    native-provider proof, ruff, mypy
  - Status: checkpointed in `dc999418`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green custom S3 proof (`1 failed`
    before implementation, then `1 passed`), adjacent native-provider proof
    (`13 passed, 376 deselected`), `ruff check
    src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, and `mypy
    src\openzues\services\ops_mesh.py`.

- [x] `OZ-PROV-001DG` Tlon DM inbound firehose session routing
  ...
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: parse upstream `chat` firehose DM events, render rich inline
    Tlon content to agent text, build the direct Tlon conversation
    target/session key, dispatch through session-backed delivery, and preserve
    inbound plus delivery message ids.
  - Evidence required: focused DM inbound session test, adjacent
    provider/session proof, ruff, mypy
...
- [x] `OZ-PROV-001DI` Tlon inbound image media staging
  ...
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: extract capped image blocks, fetch through a fakeable native
    inbound media fetcher, persist bytes in the inbound attachment store,
    annotate the session message, and return staged media metadata.
  - Evidence required: focused inbound media staging test, paired inbound
    proof, adjacent provider/session proof, ruff, mypy
  - Status: checkpointed in `d7556229`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green Tlon inbound media proof (`1
...
- [x] `OZ-PROV-001DJ` Tlon inbound authorization and pending approvals
  ...
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: enforce configured DM allowlists and restricted channel rules
    before session/media processing, preserve owner bypass semantics through
    pending approval queueing, persist native pending approvals, and keep
    blocked messages out of session delivery.
  - Evidence required: focused DM/channel authorization tests, adjacent
    provider/session proof, ruff, mypy
  ...
  - Last verified: 2026-05-08, focused Tlon authorization cluster (`5
    ...
    passed, 378 deselected`), `ruff check
    src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, and `mypy
    src\openzues\services\ops_mesh.py`.

- [x] `OZ-PROV-001DK` Tlon owner approval response replay
  ...
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: intercept owner `approve <id>` DMs before normal delivery,
    resolve pending approvals, mutate DM/channel allowlists, remove resolved
    pending entries, and replay stored original messages into sessions.
  - Evidence required: focused owner approval response test, focused Tlon
    inbound approval/session/media proof, adjacent provider/session proof,
    ruff, mypy
...
- [x] `OZ-PROV-001DL` Tlon approval block/admin handling
  - Source: `openclaw-main/extensions/tlon/src/monitor/approval.ts`,
    `openclaw-main/extensions/tlon/src/monitor/approval-runtime.ts`,
    `openclaw-main/extensions/tlon/src/monitor/index.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: owner `block <id>` removes pending approvals and persists
    blocked ships, later blocked inbound messages are ignored before approval
    or session delivery, and owner `unblock ~ship` admin commands update the
    blocked list without normal session delivery.
  - Evidence required: focused block/admin tests, adjacent provider/session
    proof, ruff, mypy
...
- [x] `OZ-PROV-001DM` Tlon production SSE monitor lifecycle
  - Source: `openclaw-main/extensions/tlon/src/channel.runtime.ts`,
    `openclaw-main/extensions/tlon/src/monitor/index.ts`,
    `openclaw-main/extensions/tlon/src/urbit/sse-client.ts`, and
    `openclaw-main/extensions/tlon/src/settings.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: enabled route-backed Tlon accounts start a native long-running
    monitor from `OpsMeshService.start()`, authenticate to `/~/login`, create a
    Urbit channel with upstream firehose/settings/groups subscriptions,
    dispatch `channels` `/v2` and `chat` `/v3` SSE payloads into
    `handle_tlon_inbound_event`, ack stream event ids, and close via
    unsubscribe plus channel `DELETE`.
  - Evidence required: focused monitor lifecycle red/green test, native
    fake-transport proof, adjacent Tlon provider/session proof, ruff, mypy
  - Status: checkpointed in `726f03cb`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green monitor lifecycle proof (`1
    failed` before implementation, then `1 passed`), native fake-transport
    proof (`1 passed`), focused pair (`2 passed`), adjacent provider/session
    proof (`25 passed, 378 deselected`), `ruff check
    src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, `mypy
    src\openzues\services\ops_mesh.py`, and focused `git diff --check`.

- [x] `OZ-PROV-001DN` `channels.start` native Tlon runtime start
  ...
  - Target: `src/openzues/services/gateway_node_methods.py`,
    `src/openzues/services/ops_mesh.py`, `src/openzues/app.py`,
    `tests/test_gateway_node_methods.py`, `tests/test_ops_mesh.py`,
    `tests/test_gateway_nodes_api.py`
  - Contract: `channels.start` validates channel/account params, maps blank
    `accountId` to `default`, dispatches through a fakeable start adapter,
    returns `{channel, accountId, started}`, preserves the unsupported runtime
    error for channels without a native start owner, and production-wires Tlon
    starts to route-backed SSE monitor startup.
  - Evidence required: focused gateway method red/green test, focused OpsMesh
    Tlon start red/green test, existing API unsupported-boundary proof,
    adjacent channel start/stop/logout proof, adjacent Tlon monitor proof,
    ruff, mypy
  - Status: checkpointed in `810a6af0`
  - Weight: 1
  - Last verified: 2026-05-08, focused gateway and OpsMesh red/green proofs
    (`1 failed` each before implementation, then `1 passed` each), API
    ...
    deselected`), `ruff check src\openzues\services\gateway_node_methods.py
    src\openzues\services\ops_mesh.py src\openzues\app.py
...
- [x] `OZ-PROV-001DO` `channels.stop` native Tlon runtime stop
  - Source: `openclaw-main/src/gateway/server-methods/channels.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_node_methods.py`,
    `src/openzues/services/ops_mesh.py`, `src/openzues/app.py`,
    `tests/test_gateway_node_methods.py`, `tests/test_ops_mesh.py`
  - Contract: `channels.stop` still supports the existing idempotent stopped
    response, but when a runtime stop adapter is wired it dispatches with the
    normalized channel/default account, returns `{channel, accountId, stopped}`,
    and production-wired Tlon stops close the matching SSE monitor handle.
  - Evidence required: focused gateway method red/green test, focused OpsMesh
    Tlon stop red/green test, existing idempotent stop proof, adjacent channel
    start/stop/logout proof, adjacent Tlon monitor proof, ruff, mypy
  - Status: checkpointed in `1365c028`
  - Weight: 1
  - Last verified: 2026-05-08, focused gateway and OpsMesh red/green proofs
    (`1 failed` each before implementation, then `1 passed` each), idempotent
    stop proof (`1 passed`), adjacent gateway method proof (`9 passed, 1118
...
- [x] `OZ-PROV-001DP` Telegram `channels.logout` runtime config cleanup
  - Source: `openclaw-main/src/gateway/server-methods/channels.ts`,
  ...
  - Target: `src/openzues/services/gateway_node_methods.py`,
    ...
    `tests/test_gateway_node_methods.py`, `tests/test_ops_mesh.py`,
    `tests/test_gateway_nodes_api.py`
  - Contract: `channels.logout` keeps the exact unsupported-channel response
    when no logout service is registered, but when app-wired for Telegram it
    normalizes blank `accountId` to `default`, stops the runtime account
    idempotently, clears saved `channels.telegram.botToken`, and returns
    OpenClaw-shaped `{channel, accountId, cleared, envToken, loggedOut}`.
  - Evidence required: focused gateway method red/green test, focused OpsMesh
    Telegram config cleanup red/green test, adjacent channel start/stop/logout
    proof, adjacent OpsMesh lifecycle proof, adjacent API proof, ruff, mypy
  - Status: checkpointed in `2d26bdc4`
  - Weight: 1
  - Last verified: 2026-05-08, focused gateway and OpsMesh red/green proofs
    (`1 failed` each before implementation, then `1 passed` each), adjacent
    gateway method proof (`10 passed, 1118 deselected`), adjacent OpsMesh
    lifecycle proof (`5 passed, 401 deselected`), adjacent API proof (`4
    passed, 424 deselected`), `ruff check
    src\openzues\services\gateway_node_methods.py
    src\openzues\services\ops_mesh.py src\openzues\app.py
...
- [x] `OZ-PROV-001DQ` LINE `channels.logout` runtime config cleanup
  - Source: `openclaw-main/src/gateway/server-methods/channels.ts`,
  ...
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: app-wired `channels.logout` for LINE stops the runtime account
    idempotently, clears saved `channels.line.channelAccessToken`,
    `channelSecret`, `tokenFile`, `secretFile`, and matching `accounts.default`
    secret fields, preserves non-secret settings, and returns OpenClaw-shaped
    `{channel, accountId, cleared, envToken, loggedOut}`.
  - Evidence required: focused OpsMesh LINE config cleanup red/green test,
    paired Telegram regression proof, adjacent OpsMesh lifecycle proof,
    adjacent gateway method proof, adjacent API proof, ruff, mypy
  - Status: checkpointed in `9674493d`
  - Weight: 1
  - Last verified: 2026-05-08, focused OpsMesh LINE red/green proof (`1
    failed` before implementation, then `1 passed`), paired Telegram
    regression proof (`1 passed`), adjacent OpsMesh lifecycle proof (`4
    passed, 403 deselected`), adjacent gateway method proof (`5 passed, 1123
    deselected`), adjacent API proof (`4 passed, 424 deselected`), `ruff check
    src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, `mypy
    src\openzues\services\ops_mesh.py`, and focused `git diff --check`.

- [x] `OZ-PROV-001DR` Nextcloud Talk `channels.logout` runtime config cleanup
  - Source: `openclaw-main/src/gateway/server-methods/channels.ts`,
  ...
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: app-wired `channels.logout` for Nextcloud Talk stops the runtime
    account idempotently, clears saved `channels.nextcloud-talk.botSecret` and
    matching `accounts.default.botSecret` fields, preserves non-secret
    `baseUrl`, and returns OpenClaw-shaped
    `{channel, accountId, cleared, envSecret, loggedOut}`.
  - Evidence required: focused OpsMesh Nextcloud Talk config cleanup red/green
    test, paired LINE/Telegram regression proof, adjacent OpsMesh lifecycle
    proof, adjacent gateway method proof, adjacent API proof, ruff, mypy
  - Status: checkpointed in `13de6593`
  - Weight: 1
  - Last verified: 2026-05-08, focused OpsMesh Nextcloud Talk red/green proof
    (`1 failed` before implementation, then `1 passed`), paired LINE/Telegram
...
- [x] `OZ-PROV-001DS` WhatsApp `channels.logout` runtime auth cleanup
  - Source: `openclaw-main/src/gateway/server-methods/channels.ts`,
    `openclaw-main/extensions/whatsapp/src/channel.ts`,
    `openclaw-main/extensions/whatsapp/src/accounts.ts`,
    `openclaw-main/extensions/whatsapp/src/auth-store.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`,
    `tests/test_ops_mesh.py`
  - Contract: app-wired `channels.logout` for WhatsApp stops the runtime
    account idempotently, resolves account auth state from
    `channels.whatsapp.authDir` or `OPENCLAW_OAUTH_DIR/whatsapp/<account>`,
    clears only managed Web-auth `creds.json`/backup-backed directories,
    preserves legacy default-auth-file cleanup, skips external or
    symlink-crossing paths, and returns OpenClaw-shaped
    `{channel, accountId, cleared, loggedOut}`.
  - Evidence required: focused OpsMesh WhatsApp auth cleanup red/green test,
    adjacent OpsMesh lifecycle proof, adjacent gateway method proof, adjacent
    API proof, ruff, mypy
  - Status: checkpointed in `3e99a587`
  - Weight: 1
  - Last verified: 2026-05-08, focused OpsMesh WhatsApp red/green proof (`1
    failed` before implementation, then `1 passed`), adjacent OpsMesh lifecycle
    proof (`6 passed, 403 deselected`), adjacent gateway method proof (`5
    passed, 1123 deselected`), adjacent API proof (`4 passed, 424
    deselected`), `ruff check src\openzues\services\ops_mesh.py
    tests\test_ops_mesh.py`, `mypy src\openzues\services\ops_mesh.py`, and
    focused `git diff --check`.

- [x] `OZ-PROV-001DT` QQBot `channels.logout` runtime config cleanup
  - Source: `openclaw-main/src/gateway/server-methods/channels.ts`,
    `openclaw-main/extensions/qqbot/src/channel.ts`,
    `openclaw-main/extensions/qqbot/src/engine/config/credentials.ts`,
    `openclaw-main/extensions/qqbot/src/bridge/config.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_node_methods.py`,
    `src/openzues/services/gateway_config.py`,
    `src/openzues/services/ops_mesh.py`,
    `tests/test_gateway_node_methods.py`, `tests/test_ops_mesh.py`
  - Contract: `qqbot`/`qq` become recognized gateway/config channel aliases;
    app-wired `channels.logout` for QQBot clears saved
    ...
    `appId`, and returns OpenClaw-shaped
    `{channel, accountId, ok, cleared, envToken, loggedOut}`.
  - Evidence required: focused OpsMesh QQBot config cleanup red/green test,
    focused gateway method acceptance proof, adjacent OpsMesh lifecycle proof,
    adjacent gateway method proof, adjacent API proof, ruff, mypy
  - Status: checkpointed in `aac53b3b`
  - Weight: 1
  - Last verified: 2026-05-08, focused OpsMesh QQBot red/green proof (`1
    failed` before implementation, then `1 passed`), focused gateway method
    acceptance proof (`1 passed`), adjacent OpsMesh lifecycle proof (`7 passed,
    403 deselected`), adjacent gateway method proof (`6 passed, 1123
    deselected`), adjacent API proof (`4 passed, 424 deselected`), `ruff check
    src\openzues\services\ops_mesh.py
    src\openzues\services\gateway_node_methods.py
...
- [x] `OZ-PROV-001DU` Zalo user `channels.logout` runtime profile cleanup
  - Source: `openclaw-main/src/gateway/server-methods/channels.ts`,
    `openclaw-main/extensions/zalouser/src/channel.ts`,
    `openclaw-main/extensions/zalouser/src/accounts.ts`,
  ...
  - Target: `src/openzues/services/gateway_node_methods.py`,
    ...
    `src/openzues/services/ops_mesh.py`,
    `tests/test_gateway_node_methods.py`, `tests/test_ops_mesh.py`
  - Contract: `zalouser`/`zalo-user` become recognized gateway/config channel
    aliases distinct from the Zalo Bot API `zalo` route; app-wired
    `channels.logout` for Zalo user resolves profile precedence from
    config/env/account/default, deletes the matching
    `plugin-state/credentials/zalouser/credentials*.json` file, and returns
    OpenClaw-shaped `{channel, accountId, profile, cleared, loggedOut,
    message}`.
  - Evidence required: focused OpsMesh Zalo user profile cleanup red/green
    test, focused gateway method acceptance proof, adjacent OpsMesh lifecycle
    proof, adjacent gateway method proof, adjacent API proof, ruff, mypy
  - Status: checkpointed in `4d67d5a6`
  - Weight: 1
  - Last verified: 2026-05-08, focused OpsMesh Zalo user red/green proof (`1
    failed` before implementation, then `1 passed`), focused gateway method
    acceptance proof (`1 failed` before implementation, then `1 passed`),
    adjacent OpsMesh lifecycle proof (`8 passed, 403 deselected`), adjacent
    gateway method proof (`7 passed, 1123 deselected`), adjacent API proof (`4
    passed, 424 deselected`), `ruff check src\openzues\services\ops_mesh.py
    src\openzues\services\gateway_node_methods.py
    src\openzues\services\gateway_config.py tests\test_ops_mesh.py
    tests\test_gateway_node_methods.py`, `mypy
    src\openzues\services\ops_mesh.py
...
- [x] `OZ-PROV-001DA` iMessage config-backed CLI/RPC account probe
  - Source: `openclaw-main/extensions/imessage/src/probe.ts`,
    `openclaw-main/extensions/imessage/src/status-core.ts`,
    `openclaw-main/extensions/imessage/src/accounts.ts`,
    `openclaw-main/extensions/imessage/src/client.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_channels.py`,
    `src/openzues/services/ops_mesh.py`, `src/openzues/app.py`,
    `src/openzues/cli.py`, `tests/test_cli.py`,
    `tests/test_ops_mesh.py`, `tests/test_gateway_node_methods.py`
  - Contract: `channels status --probe --json` discovers configured
    `channels.imessage` accounts from Gateway config, checks the configured
    `imsg` binary, probes `imsg rpc --help` with the upstream fatal old-CLI
    guard, starts `imsg rpc` with `--db` when configured, sends JSON-RPC
    `chats.list` `{limit: 1}`, and returns native-CLI-backed account probe
    metadata.
  - Evidence required: focused iMessage channel-probe CLI test, focused
    RPC-support fatal guard test, adjacent channel-probe CLI tests, adjacent
    ops tests, gateway owner channels-status proof, ruff, mypy
  - Status: checkpointed in `86d0b06b`
  - Weight: 1
  - Last verified: 2026-05-08, focused
    `python -m pytest tests\test_cli.py::test_channels_status_json_uses_configured_imessage_probe tests\test_ops_mesh.py::test_ops_mesh_service_imessage_rpc_support_marks_unknown_subcommand_fatal -q`
    (`2 passed`), adjacent channel-probe proof (`3 passed, 520
    deselected`), adjacent ops proof (`3 passed, 378 deselected`), gateway
    owner proof (`2 passed, 1123 deselected`), `ruff check
    src\openzues\services\ops_mesh.py src\openzues\services\gateway_channels.py
    src\openzues\app.py src\openzues\cli.py tests\test_cli.py
    tests\test_ops_mesh.py tests\test_gateway_node_methods.py`, and `mypy
    src\openzues\services\ops_mesh.py src\openzues\services\gateway_channels.py
    src\openzues\app.py src\openzues\cli.py`.

- [x] `OZ-PROV-001CZ` Tlon route-backed account probe
  - Source: `openclaw-main/extensions/tlon/src/channel.ts`,
    `openclaw-main/extensions/tlon/src/channel.runtime.ts`,
    `openclaw-main/extensions/tlon/src/urbit/auth.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`,
    `src/openzues/services/gateway_channels.py`, `src/openzues/schemas.py`,
    `src/openzues/cli.py`, `tests/test_cli.py`, `tests/test_ops_mesh.py`
  - Contract: `channels status --probe --json` treats saved `kind="tlon"`
    routes as probeable, resolves ship URL and ship id from the route target,
    posts the route access-code secret to `/~/login`, carries the returned
    auth cookie into `/~/name`, preserves the upstream name HTTP status as
    `httpStatus`, and returns a native-provider-backed probe envelope without
    enabling Tlon as a send-capable route fallback.
  - Evidence required: focused Tlon channel-probe CLI test, focused Tlon
    login/name helper test, adjacent channel-probe CLI tests, adjacent
    provider probe tests, ruff, mypy
  - Status: checkpointed in `dd613729`
  - Weight: 1
  - Last verified: 2026-05-08, focused
    `python -m pytest tests\test_cli.py::test_channels_status_json_uses_route_backed_tlon_probe tests\test_ops_mesh.py::test_ops_mesh_service_tlon_probe_authenticates_then_requests_name -q`
    (`2 passed`), adjacent channel-probe proof (`3 passed, 519
    deselected`), adjacent ops proof (`3 passed, 377 deselected`), `ruff
    check src\openzues\services\ops_mesh.py src\openzues\services\gateway_channels.py
    src\openzues\schemas.py src\openzues\cli.py tests\test_cli.py
    tests\test_ops_mesh.py`, and `mypy src\openzues\services\ops_mesh.py
    src\openzues\services\gateway_channels.py src\openzues\schemas.py
    src\openzues\cli.py`.

- [x] `OZ-PROV-001CY` BlueBubbles route-backed account probe
  ...
  - Contract: `channels status --probe --json` treats native
    `kind="bluebubbles"` routes as probeable, calls `/api/v1/ping` through the
    saved server URL/password, preserves the upstream ping HTTP status as
    `httpStatus`, and returns a native-provider-backed probe envelope with
    account, base URL, timeout, and HTTP status metadata.
  - Evidence required: focused BlueBubbles channel-probe CLI test, focused
    HTTP status helper test, adjacent channel-probe CLI tests, adjacent
    BlueBubbles route/media tests, ruff, mypy
  ...
  - Last verified: 2026-05-08, focused
    ...
    (`2 passed`), adjacent channel-probe proof (`15 passed, 506
    deselected`), adjacent BlueBubbles ops proof (`6 passed, 373 deselected`),
    `ruff check src\openzues\services\ops_mesh.py tests\test_cli.py
    tests\test_ops_mesh.py`, and `mypy src\openzues\services\ops_mesh.py`.

- [x] `OZ-PROV-001CX` Twitch route-backed account probe
  ...
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_cli.py`,
    `tests/test_ops_mesh.py`
  - Contract: `channels status --probe --json` treats native
    `kind="twitch"` routes as probeable, parses saved `twitch://chat`
    credentials, opens Twitch IRC over TLS, sends OAuth PASS/NICK, answers
    PING, waits for Twitch `001`, quits with `QUIT :probe`, and returns a
    native-provider-backed probe envelope with account, username, channel,
    connected, timeout, and `elapsedMs` metadata.
  - Evidence required: focused Twitch channel-probe CLI test, focused Twitch
    runtime helper test, adjacent channel-probe CLI tests, adjacent Twitch ops
    tests, ruff, mypy
  ...
  - Last verified: 2026-05-08, focused
    ...
    (`2 passed`), adjacent channel-probe proof (`14 passed, 506
    deselected`), adjacent Twitch ops proof (`3 passed, 375 deselected`),
    `ruff check src\openzues\services\ops_mesh.py tests\test_cli.py
    tests\test_ops_mesh.py`, and `mypy src\openzues\services\ops_mesh.py`.

- [x] `OZ-PROV-001CW` IRC route-backed account probe
  - Source: `openclaw-main/extensions/irc/src/probe.ts`,
    `openclaw-main/extensions/irc/src/channel.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_cli.py`,
    `tests/test_ops_mesh.py`
  - Contract: `channels status --probe --json` treats native `kind="irc"`
    routes as probeable, parses saved `irc(s)://` route credentials, sends
    PASS/NICK/USER, answers PING, waits for IRC `001`, quits with
    `QUIT :probe`, and returns a native-provider-backed probe envelope with
    account, host, port, TLS, nick, timeout, and latency metadata.
  - Evidence required: focused IRC channel-probe CLI test, focused IRC
    runtime helper test, adjacent channel-probe CLI tests, adjacent IRC ops
    tests, ruff, mypy
  ...
  - Last verified: 2026-05-08, focused
    ...
    (`2 passed`), adjacent channel-probe proof (`13 passed, 506
    deselected`), adjacent IRC ops proof (`2 passed, 375 deselected`), `ruff
    check src\openzues\services\ops_mesh.py tests\test_cli.py
    tests\test_ops_mesh.py`, and `mypy src\openzues\services\ops_mesh.py`.

- [x] `OZ-PROV-001CV` Signal route-backed account probe
  - Source: `openclaw-main/extensions/signal/src/probe.ts`,
    `openclaw-main/extensions/signal/src/channel.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_cli.py`
  - Contract: `channels status --probe --json` treats native
    `kind="signal"` routes as probeable without a route secret, calls
    `/api/v1/check`, then calls JSON-RPC `version`, and returns a
    native-provider-backed probe envelope with account, timeout, HTTP status,
    and daemon version metadata.
  - Evidence required: focused Signal channel-probe CLI test, adjacent
...
- [x] `OZ-PROV-001CU` Mattermost route-backed account probe
  - Source: `openclaw-main/extensions/mattermost/src/mattermost/probe.ts`,
    `openclaw-main/extensions/mattermost/src/channel.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_cli.py`
  - Contract: `channels status --probe --json` treats native
    `kind="mattermost"` routes as probeable, normalizes route targets with or
    without `/api/v4`, calls Mattermost `users/me` through the saved bot token,
    and returns a native-provider-backed probe envelope with account, timeout,
    and bot user metadata.
  - Evidence required: focused Mattermost channel-probe CLI test, adjacent
    channel-probe CLI tests, ruff, mypy
...
- [x] `OZ-PROV-001CT` Feishu/Lark route-backed account probe
  - Source: `openclaw-main/extensions/feishu/src/probe.ts`,
    `openclaw-main/extensions/feishu/src/channel.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/ops_mesh.py`, `tests/test_cli.py`
  - Contract: `channels status --probe --json` treats native
    `kind="feishu"` routes as probeable, posts `needBotInfo=true` to the
    Feishu/Lark Open API `bot/v1/openclaw_bot/ping` endpoint through the saved
    bearer token, and returns a native-provider-backed probe envelope with
    account, timeout, bot name, and bot open-id metadata.
  - Evidence required: focused Feishu/Lark channel-probe CLI test, adjacent
    channel-probe CLI tests, ruff, mypy
...
- [x] `OZ-PLUGIN-00302` Imported extension-shared utility facade shim
  - Source: `openclaw-main/src/plugin-sdk/extension-shared.ts`,
    `openclaw-main/src/utils/zod-parse.ts`,
    `openclaw-main/src/utils/fetch-timeout.ts`,
    `openclaw-main/src/infra/net/proxy-env.ts`,
    `openclaw-main/src/secrets/ref-contract.ts`,
    `openclaw-main/src/plugin-sdk/channel-lifecycle.core.ts`, and
  ...
  - Contract: native installed plugin runtimes can require scoped and
    unscoped `extension-shared` and receive the exact utility barrel for schema
    ...
    deferred promise creation, plugin config issue mapping, read-only env
    secret provider gates, package-version candidate resolution, and no-proxy
    ambient proxy-agent resolution.
  - Evidence required: focused gateway method test, adjacent plugin invoke
    tests, ruff, mypy
  - Status: checkpointed in `b56d15d7`
  - Weight: 1
  - Last verified: 2026-05-07, focused extension-shared proof (`1 passed`),
    adjacent extension/discord/runtime/fetch/secret proof (`5 passed, 1120
    deselected`), adjacent imported-plugin proof (`313 passed, 812
    deselected`), `ruff check`, `mypy`, and `git diff --check`.

- [x] `OZ-PLUGIN-00301` Imported discord facade shim
  - Source: `openclaw-main/src/plugin-sdk/discord.ts`,
    `openclaw-main/src/plugin-sdk/channel-contract.js`,
    `openclaw-main/src/plugin-sdk/channel-core.js`,
    `openclaw-main/src/plugin-sdk/config-types.js`,
    `openclaw-main/src/plugin-sdk/channel-plugin-common.js`,
    `openclaw-main/src/plugin-sdk/channel-status.js`,
    `openclaw-main/src/plugin-sdk/bundled-channel-config-schema.js`, and
    bundled Discord public surface contracts for `api.js` and `runtime-api.js`
  - References: `openclaw-main/src/plugin-sdk/discord.test.ts`
  - Target: `src/openzues/cli.py`, `tests/test_gateway_node_methods.py`
  - Contract: native installed plugin runtimes can require scoped and
    unscoped `discord` and receive the deprecated Discord compatibility barrel
    for channel-common helpers, `DiscordConfigSchema`, status helpers,
    ...
    group mention/tool policy resolution, status issue projection,
    runtime-config filled subagent thread auto-binding, thread binding
    list/unbind helpers, and fakeable bundled Discord public-surface
    delegation.
  - Evidence required: focused gateway method test, adjacent plugin invoke
    tests, ruff, mypy
  - Status: checkpointed in `307777d8`
  - Weight: 1
  - Last verified: 2026-05-07, focused discord facade proof (`1 passed`),
    adjacent discord/compat/channel-status/channel-plugin proof (`4 passed,
    1120 deselected`), adjacent imported-plugin proof (`312 passed, 812
    deselected`), `ruff check`, `mypy`, and `git diff --check`.
...
- [x] `OZ-PLUGIN-00299` Imported channel-runtime compatibility facade shim
  - Source: `openclaw-main/src/plugin-sdk/channel-runtime.ts`,
    `openclaw-main/src/channels/chat-type.ts`,
    `openclaw-main/src/channels/reply-prefix.ts`,
    `openclaw-main/src/channels/typing.ts`,
    `openclaw-main/src/channels/plugins/registry.ts`,
    ...
    `openclaw-main/src/infra/transport-ready.ts`, and
    `openclaw-main/src/plugin-sdk/channel-lifecycle.core.ts`
  - References: none
  - Target: `src/openzues/cli.py`, `tests/test_gateway_node_methods.py`
  - Contract: native installed plugin runtimes can require scoped and
    unscoped `channel-runtime` and receive the exact public compatibility
    barrel for channel chat type normalization, reply prefix/typing helpers,
  ...
  - Weight: 1
  - Last verified: 2026-05-07, focused channel-runtime proof (`1 passed`),
    adjacent channel-runtime/context/activity/heartbeat/poll/transport-ready
    proof (`7 passed, 1115 deselected`), adjacent imported-plugin proof
    (`310 passed, 812 deselected`), `ruff check`, `mypy`, and
    `git diff --check`.
...
- [x] `OZ-COMP-001G` QR human approval instructions
  - Source: `openclaw-main/src/cli/qr-cli.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: human `openzues qr --no-ascii --url ...` output appends
    OpenClaw's post-scan approval block after setup metadata, using the native
    `openzues devices list` and
    `openzues devices approve <requestId>` command names.
  - Evidence required: focused QR human-output test, adjacent QR tests, ruff,
    mypy
...
- [x] `OZ-PKG-001F` source-install package doctor warnings
  - Source: `openclaw-main/src/commands/doctor-install.ts`,
    `openclaw-main/src/flows/doctor-health.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues doctor --json` emits
    `packageDistribution.sourceInstall` for pnpm source workspaces, warning
    about non-pnpm `node_modules`, stray `package-lock.json`, and missing
    `node_modules/.bin/tsx` while preserving dist-inventory diagnostics.
  - Evidence required: focused source-install doctor test, paired package
    distribution regressions, adjacent update/package doctor proof, ruff, mypy
  - Status: checkpointed in `4c1d7a2a`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green source-install doctor proof
    (`1 failed` before implementation, then `1 passed`), paired package
    distribution regressions (`2 passed`), adjacent update/package doctor
    proof (`7 passed, 517 deselected`), `ruff check src\openzues\cli.py
    tests\test_cli.py`, `mypy src\openzues\cli.py`, and focused
    `git diff --check`.

- [x] `OZ-PKG-001G` update-status git-tag channel projection
  - Source: `openclaw-main/src/infra/update-channels.ts`,
    `openclaw-main/src/cli/update-cli/status.ts`,
    `openclaw-main/src/commands/status.update.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues update status --json` gives exact git tags precedence
    over branch/default channel inference, projects `channel.source="git-tag"`,
    and derives `beta` vs `stable` labels from the tag while preserving config
    channel precedence.
  - Evidence required: focused git-tag update-status test, adjacent
    update/package doctor proof, ruff, mypy
  - Status: checkpointed in `dce24b5e`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green git-tag update-status proof
    (`1 failed` before implementation, then `1 passed`), adjacent
    update/package doctor proof (`7 passed, 518 deselected`), `ruff check
    src\openzues\cli.py tests\test_cli.py`, `mypy src\openzues\cli.py`, and
    focused `git diff --check`.

- [x] `OZ-PKG-001H` packed git-tag update-status channel projection
  - Source: `openclaw-main/src/infra/update-check.ts`,
    `openclaw-main/src/infra/update-channels.ts`,
    `openclaw-main/src/cli/update-cli/status.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues update status --json` recognizes exact git tags stored
    in `.git/packed-refs` and projects the same `git-tag` stable/beta channel
    metadata as loose `.git/refs/tags` refs.
  - Evidence required: focused packed git-tag update-status test, adjacent
    update/package doctor proof, ruff, mypy
  - Status: checkpointed in `71029a02`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green packed git-tag update-status
...
- [x] `OZ-PKG-001I` update-status git metadata envelope
  - Source: `openclaw-main/src/infra/update-check.ts`,
    `openclaw-main/src/cli/update-cli/status.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues update status --json` includes OpenClaw-shaped
    `update.git` metadata for git installs with root, sha, tag, branch, and
    null upstream/dirty/ahead/behind/fetch placeholders when native local
    parsing cannot prove those fields.
  - Evidence required: focused git metadata update-status test, adjacent
    update/package doctor proof, ruff, mypy
  - Status: checkpointed in `b31d8f41`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green git metadata update-status
    proof (`1 failed` before implementation, then `1 passed`), adjacent
    update/package doctor proof (`9 passed, 518 deselected`), `ruff check
    src\openzues\cli.py tests\test_cli.py`, `mypy src\openzues\cli.py`, and
    focused `git diff --check`.

- [x] `OZ-PKG-001J` update-status registry availability projection
  - Source: `openclaw-main/src/commands/status.update.ts`,
    `openclaw-main/src/infra/update-check.ts`,
    `openclaw-main/src/cli/update-cli/status.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues update status --json` preserves live
    `update.registry.latestVersion` metadata and projects newer registry
    versions into OpenClaw-shaped availability fields.
  - Evidence required: focused registry availability update-status test,
    adjacent update/package doctor proof, ruff, mypy
  - Status: checkpointed in `55a785a8`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green registry availability
    update-status proof (`1 failed` before implementation, then `1 passed`),
...
- [x] `OZ-PKG-001K` update-status git availability projection
  - Source: `openclaw-main/src/commands/status.update.ts`,
    `openclaw-main/src/infra/update-check.ts`,
    `openclaw-main/src/cli/update-cli/status.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues update status --json` preserves live
    `update.git` divergence metadata and projects positive `behind` counts into
    OpenClaw-shaped availability fields.
  - Evidence required: focused git availability update-status test, adjacent
    update/package doctor proof, ruff, mypy
  - Status: checkpointed in `de37e6f8`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green git availability
    update-status proof (`1 failed` before implementation, then `1 passed`),
    adjacent update/package doctor proof (`11 passed, 518 deselected`), `ruff
    check src\openzues\cli.py tests\test_cli.py`, `mypy src\openzues\cli.py`,
    and focused `git diff --check`.

- [x] `OZ-PKG-001L` update-status config channel precedence over git tag
  - Source: `openclaw-main/src/infra/update-channels.ts`,
    `openclaw-main/src/cli/update-cli/status.ts`
  - References: Hermes/Warp `none`
  - Target: `tests/test_cli.py`
  - Contract: `openzues update status --json` keeps configured update channels
    authoritative over git-tag inference while preserving `update.git.tag`
    metadata.
  - Evidence required: focused config-over-tag update-status test, adjacent
    update-status proof, ruff
  ...
  - Last verified: 2026-05-08, focused config-over-tag update-status proof
    ...
    `ruff check tests\test_cli.py`, and focused `git diff --check`.

- [x] `OZ-PKG-001M` human update-status update-available hint
  - Source: `openclaw-main/src/commands/status.update.ts`,
    `openclaw-main/src/cli/update-cli/status.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: human `openzues update status` emits an actionable
    update-available hint with registry/git details when computed availability
    is true.
  - Evidence required: focused human hint update-status test, adjacent
    update-status proof, ruff, mypy
  - Status: checkpointed in `20e9c885`
  - Weight: 1
  - Last verified: 2026-05-08, focused red/green human update hint proof (`1
    failed` before implementation, then `1 passed`), adjacent update-status
    proof (`10 passed, 521 deselected`), `ruff check src\openzues\cli.py
    tests\test_cli.py`, `mypy src\openzues\cli.py`, and focused
    `git diff --check`.

- [x] `OZ-PKG-001N` human update-status git-behind hint proof
  - Source: `openclaw-main/src/commands/status.update.ts`,
    `openclaw-main/src/cli/update-cli/status.ts`
  - References: Hermes/Warp `none`
  - Target: `tests/test_cli.py`
  - Contract: human `openzues update status` emits the OpenClaw-shaped
    `git behind N` hint detail when git availability is true.
  - Evidence required: focused human git hint update-status test, adjacent
    human/update proof, ruff
  ...
  - Last verified: 2026-05-08, focused human git update hint proof (`1
    ...
    check tests\test_cli.py`, and focused `git diff --check`.

- [x] `OZ-PKG-001O` package dist inventory drift diagnostics
  - Source: `openclaw-main/src/infra/package-dist-inventory.ts`,
    `openclaw-main/src/infra/package-dist-inventory.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues doctor --json` compares valid
    `dist/postinstall-inventory.json` entries against packaged `dist/` files
    and reports OpenClaw-shaped `missing packaged dist file ...` and
    `unexpected packaged dist file ...` warnings when the inventory is stale.
  - Evidence required: focused package-dist drift doctor test, adjacent package
    doctor proof, ruff, mypy
...
- [x] `OZ-PKG-001P` package dist legacy staging-debris diagnostics
  - Source: `openclaw-main/src/infra/package-dist-inventory.ts`,
    `openclaw-main/src/infra/package-dist-inventory.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues doctor --json` detects legacy plugin dependency
    staging directories under packaged
    `dist/extensions/*/.openclaw-install-stage*` paths and emits the upstream
    `unexpected legacy plugin dependency staging debris in package dist: ...`
    warning plus a package distribution check.
  - Evidence required: focused staging-debris doctor test, adjacent package
    doctor proof, ruff, mypy
...
- [x] `OZ-PKG-001Q` mixed-case package dist staging-debris proof
  - Source: `openclaw-main/src/infra/package-dist-inventory.ts`,
    `openclaw-main/src/infra/package-dist-inventory.test.ts`
  - References: Hermes/Warp `none`
  - Target: `tests/test_cli.py`
  - Contract: `openzues doctor --json` preserves OpenClaw's case-insensitive
    staging-debris path matching across `Dist`, `Extensions`, and
    `.OPENCLAW-INSTALL-STAGE-*` path segments.
  - Evidence required: focused mixed-case staging-debris doctor test, adjacent
    package doctor proof, ruff
...
- [x] `OZ-PKG-001R` exact missing package dist inventory warning
  - Source: `openclaw-main/src/infra/package-dist-inventory.ts`,
    `openclaw-main/src/infra/package-dist-inventory.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues doctor --json` reports OpenClaw's exact
    `missing package dist inventory dist/postinstall-inventory.json` warning
    when a packaged install has `dist/` but no postinstall inventory.
  - Evidence required: focused missing-inventory doctor test, adjacent package
    doctor proof, ruff, mypy
...
- [x] `OZ-PKG-001S` package dist local metadata/dependency omission
  - Source: `openclaw-main/src/infra/package-dist-inventory.ts`,
    `openclaw-main/src/infra/package-dist-inventory.test.ts`,
    `openclaw-main/scripts/lib/local-build-metadata-paths.mjs`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: package dist inventory comparison omits local build metadata
    stamps, source maps, root extension `node_modules`, and per-extension
    dependency `node_modules` trees while still comparing real packaged files.
  - Evidence required: focused omission doctor test, adjacent package doctor
    proof, ruff, mypy
...
- [x] `OZ-PKG-001T` unsafe package dist symlink diagnostics
  - Source: `openclaw-main/src/infra/package-dist-inventory.ts`,
    `openclaw-main/src/infra/package-dist-inventory.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues doctor --json` detects symlinked packaged `dist/`
    entries and reports OpenClaw's exact `Unsafe package dist path: ...`
    warning.
  - Evidence required: focused unsafe-symlink doctor test, adjacent package
    doctor proof, ruff, mypy
...
- [x] `OZ-PKG-001U` externalized bundled extension dist omission
  - Source: `openclaw-main/src/infra/package-dist-inventory.ts`,
    `openclaw-main/src/infra/package-dist-inventory.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: package dist inventory comparison omits publishable externalized
    bundled extension dist trees while preserving bundled and
    `bundle.includeInCore=true` extension runtime files.
  - Evidence required: focused externalized-extension omission doctor test,
...
- [x] `OZ-PKG-001V` private QA package dist artifact omission
  - Source: `openclaw-main/src/infra/package-dist-inventory.ts`,
    `openclaw-main/src/infra/package-dist-inventory.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: package dist inventory comparison omits private QA extension
    dist trees, private QA plugin-sdk files/prefixes, and `dist/qa-runtime-*`
    chunks while still comparing real packaged files.
  - Evidence required: focused private-QA omission doctor test, adjacent
    package doctor proof, ruff, mypy
...
- [x] `OZ-PKG-001X` bundled runtime sidecar enforcement
  - Source: `openclaw-main/src/infra/update-global.ts`,
    `openclaw-main/src/infra/update-global.test.ts`,
    `openclaw-main/scripts/lib/bundled-runtime-sidecar-paths.json`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: package distribution diagnostics enforce critical installed
    bundled runtime sidecars even when the package inventory omits them,
    reporting `missing bundled runtime sidecar ...` for missing sidecar files.
  - Evidence required: focused bundled-sidecar doctor test, adjacent package
    doctor proof, ruff, mypy
...
- [x] `OZ-PKG-001Y` private QA bundled sidecar no-warning proof
  - Source: `openclaw-main/src/infra/update-global.ts`,
    `openclaw-main/src/infra/update-global.test.ts`,
    `openclaw-main/scripts/lib/bundled-runtime-sidecar-paths.json`
  - References: Hermes/Warp `none`
  - Target: `tests/test_cli.py`
  - Contract: package distribution diagnostics exclude private QA bundled
    plugin roots from bundled runtime sidecar enforcement.
  - Evidence required: focused private-QA sidecar doctor test, adjacent
    sidecar/QA proof, ruff
  ...
  - Last verified: 2026-05-08, focused
    ...
    (`3 passed, 541 deselected`), `ruff check tests\test_cli.py`, and
    focused `git diff --check`.

- [x] `OZ-PKG-001Z` update dry-run preview package-spec mapping
  - Source: `openclaw-main/src/cli/update-cli/update-command.ts`,
    `openclaw-main/src/infra/update-global.ts`,
    `openclaw-main/src/infra/update-global.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: root `openzues update --dry-run --json --tag main
    --no-restart` emits an OpenClaw-shaped non-mutating preview with native
    OpenZues package `main` spec mapping, planned global package manager
    action text, non-registry lookup note, restart posture, install kind,
    update install kind, and mode metadata.
  - Evidence required: focused update dry-run test, adjacent update status
    tests, ruff, mypy
  ...
  - Last verified: 2026-05-08, focused red/green
    ...
    (`1 failed` before implementation, then `1 passed`), adjacent update
...
- [x] `OZ-PKG-001AA` update-status timeout option
  - Source: `openclaw-main/src/cli/update-cli.ts`,
    `openclaw-main/src/cli/update-cli/status.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues update status --json --timeout <seconds>` accepts the
    OpenClaw status timeout option, validates it as a positive second value,
    and applies it to the live `/api/update/status` probe before falling back
    to the stored Hermes update view.
  - Evidence required: focused update-status timeout test, adjacent update
    status/dry-run tests, ruff, mypy
...
- [x] `OZ-PKG-001AB` update package-spec env override
  - Source: `openclaw-main/src/cli/update-cli/update-command.ts`,
    `openclaw-main/src/infra/update-global.ts`,
    `openclaw-main/src/infra/update-global.test.ts`
  - References: Hermes/Warp `none`
  - Target: `tests/test_cli.py`
  - Contract: `OPENCLAW_UPDATE_PACKAGE_SPEC` overrides the package target for
    `openzues update --dry-run --json`, and the override is preserved in both
    `tag` metadata and planned global package-manager action text.
  - Evidence required: focused update dry-run override test, adjacent update
    status/dry-run tests, ruff
  ...
  - Last verified: 2026-05-08, focused
    ...
    (`1 passed`), adjacent update proof
    `python -m pytest tests\test_cli.py -q -k "update_dry_run or update_status"`
    (`15 passed, 532 deselected`), `ruff check tests\test_cli.py`, and
    focused `git diff --check`.

- [x] `OZ-PKG-001AC` explicit update install-spec preservation
  - Source: `openclaw-main/src/cli/update-cli/update-command.ts`,
    `openclaw-main/src/infra/update-global.ts`,
    `openclaw-main/src/infra/update-global.test.ts`
  - References: Hermes/Warp `none`
  - Target: `tests/test_cli.py`
  - Contract: explicit package install specs, including GitHub branch specs,
    are preserved by `openzues update --dry-run --json` in `tag` metadata,
    planned global package-manager action text, and non-registry lookup notes.
  - Evidence required: focused explicit update install-spec test, adjacent
    update status/dry-run tests, ruff
  ...
  - Last verified: 2026-05-08, focused
    ...
    (`1 passed`), adjacent update proof
    `python -m pytest tests\test_cli.py -q -k "update_dry_run or update_status"`
    (`16 passed, 532 deselected`), `ruff check tests\test_cli.py`, and
    focused `git diff --check`.

- [x] `OZ-PKG-001AD` root update runtime dispatch
  - Source: `openclaw-main/src/cli/update-cli.ts`,
    `openclaw-main/src/cli/update-cli/update-command.ts`,
    `openclaw-main/src/infra/update-runner.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: root `openzues update --json --timeout <seconds>` dispatches
    through OpenZues' native `RuntimeUpdateService.run_update`, propagates the
    timeout in milliseconds, emits OpenClaw-shaped JSON update results, and
    exits nonzero for runtime update errors.
  - Evidence required: focused root update dispatch test, adjacent update
    status/dry-run tests, ruff, mypy
...
- [x] `OZ-PKG-001AE` inherited update-status parent options
  - Source: `openclaw-main/src/cli/update-cli.ts`,
    `openclaw-main/src/cli/update-cli.option-collisions.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: parent `openzues update --json --timeout <seconds> status`
    options are inherited by `update status`, making the status output JSON and
    passing the timeout through to the live update-status probe.
  - Evidence required: focused inherited-options test, adjacent update
    status/dry-run tests, ruff, mypy
...
- [x] `OZ-PKG-001AF` package update runtime path
  - Source: `openclaw-main/src/infra/package-update-steps.ts`,
    `openclaw-main/src/infra/update-global.ts`,
    `openclaw-main/src/cli/update-cli/update-command.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/runtime_updates.py`,
    `src/openzues/cli.py`, `tests/test_runtime_updates.py`, `tests/test_cli.py`
  - Contract: package-shaped root updates resolve the package manager and
    package spec, dispatch through a fakeable native package update service,
    run upstream-shaped npm/pnpm/bun global install args as a `global update`
    step, and return OpenClaw-style update result envelopes.
  - Evidence required: focused runtime package-update test, focused CLI
    package dispatch test, adjacent runtime/update tests, ruff, mypy
  - Status: checkpointed in `1291d361`
  - Weight: 2
  - Last verified: 2026-05-08, focused service/CLI proof
    `python -m pytest tests\test_cli.py::test_update_json_dispatches_package_update_service tests\test_runtime_updates.py::test_runtime_update_run_package_update_executes_global_install_step -q`
    (`2 passed`), adjacent proof
...
- [x] `OZ-PKG-001AG` npm update omit-optional fallback
  - Source: `openclaw-main/src/infra/package-update-steps.ts`,
    `openclaw-main/src/infra/update-global.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/runtime_updates.py`,
    `tests/test_runtime_updates.py`
  - Contract: when an npm package update step fails, native package updates
    retry once as `global update (omit optional)` with `--omit=optional` plus
    the quiet npm install flags, while pnpm/bun keep the single-step behavior.
  - Evidence required: focused npm fallback test, adjacent package-update
    tests, full runtime update suite, ruff, mypy
  ...
  - Last verified: 2026-05-08, focused red/green
    ...
    (`2 passed, 4 deselected`), full runtime update suite (`6 passed`),
    `ruff check src\openzues\services\runtime_updates.py
    tests\test_runtime_updates.py`, `mypy
    src\openzues\services\runtime_updates.py`, and focused `git diff --check`.

- [x] `OZ-PKG-001AH` package update version verification
  - Source: `openclaw-main/src/infra/package-update-steps.ts`,
    `openclaw-main/src/infra/update-global.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/runtime_updates.py`,
    `tests/test_runtime_updates.py`
  - Contract: explicit version package specs add a `global install verify`
    step after package-manager success and return an update error when the
    installed package version differs from the requested version.
  - Evidence required: focused version-verify test, adjacent package-update
    tests, full runtime update suite, ruff, mypy
  ...
  - Last verified: 2026-05-08, focused red/green
    ...
    (`1 failed` before implementation, then `1 passed`), adjacent
    `python -m pytest tests\test_runtime_updates.py -q -k "run_package_update"`
    (`3 passed, 4 deselected`), full runtime update suite (`7 passed`),
    `ruff check src\openzues\services\runtime_updates.py
    tests\test_runtime_updates.py`, `mypy
    src\openzues\services\runtime_updates.py`, and focused `git diff --check`.

- [x] `OZ-PKG-001AI` package update failedStep projection
  - Source: `openclaw-main/src/infra/package-update-steps.ts`,
    `openclaw-main/src/infra/update-runner.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/runtime_updates.py`,
    `tests/test_runtime_updates.py`
  - Contract: failed package/git update result envelopes include top-level
    `failedStep` metadata that points at the first failed update step.
  - Evidence required: focused failedStep assertion, full runtime update
    suite, ruff, mypy
...
- [x] `OZ-COMP-001P` Device pairing CLI mutation commands
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues devices remove`, `clear`, `reject`, `rotate`, and
    `revoke` route to `device.pair.remove`, `device.pair.list`,
    `device.pair.reject`, `device.token.rotate`, and `device.token.revoke`,
    support JSON output, guard destructive clear behind `--yes`, optionally
    reject pending requests with `--pending`, and preserve repeated `--scope`
    values for token rotation.
  - Evidence required: focused devices mutation CLI proof, adjacent
    devices/pairing CLI proof, ruff, mypy
  - Status: checkpointed in `1c25fdbb`
  - Weight: 1
  - Last verified: 2026-05-12,
    ...
    (`5 failed` before implementation, then `5 passed`), adjacent
    `python -m pytest tests\test_cli.py -q -k "devices_ or pairing_approve"`
    (`13 passed, 596 deselected`), `ruff check src\openzues\cli.py
    tests\test_cli.py`, `mypy src\openzues\cli.py`, and focused `git diff
    --check`.

- [x] `OZ-COMP-001Q` Device approve preview gateway flags
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues devices approve --latest` accepts OpenClaw-style
    remote preview flags, preserves `--url` and `--timeout` in the generated
    rerun command, projects `--token` and `--password` as
    `requiresAuthFlags`, and never leaks secret values.
  - Evidence required: focused devices approve preview proof, adjacent
    devices/pairing CLI proof, ruff, mypy
  - Status: checkpointed in `204bcdd2`
  - Weight: 1
  - Last verified: 2026-05-12,
    ...
    (`2 passed`), adjacent
    `python -m pytest tests\test_cli.py -q -k "devices_ or pairing_approve"`
    (`14 passed, 596 deselected`), `ruff check src\openzues\cli.py
    tests\test_cli.py`, `mypy src\openzues\cli.py`, and focused `git diff
    --check`.

- [x] `OZ-COMP-001R` Remote device list/approve gateway dispatch
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: explicit `--url` on `openzues devices list` and
    `openzues devices approve <requestId>` dispatches to a fakeable remote
    gateway method call, forwards auth/timeout options, preserves JSON output,
    and does not leak secret values.
  - Evidence required: focused remote devices CLI proof, adjacent
    devices/pairing CLI proof, ruff, mypy
  - Status: checkpointed in `fa6d230a`
  - Weight: 1
  - Last verified: 2026-05-12,
    `python -m pytest tests\test_cli.py::test_devices_list_json_calls_remote_gateway_with_auth_flags tests\test_cli.py::test_devices_approve_json_calls_remote_gateway_with_auth_flags -q`
    (`2 failed` before implementation because the fakeable remote hook was
    missing, then `2 passed`),
    `python -m pytest tests\test_cli.py::test_devices_list_json_calls_remote_gateway_with_auth_flags tests\test_cli.py::test_devices_approve_json_calls_remote_gateway_with_auth_flags tests\test_cli.py::test_devices_approve_latest_json_preserves_gateway_flags_without_secrets -q`
    (`3 passed`), adjacent
    `python -m pytest tests\test_cli.py -q -k "devices_ or pairing_approve"`
    (`16 passed, 596 deselected`), `ruff check src\openzues\cli.py
    tests\test_cli.py`, `mypy src\openzues\cli.py`, and focused `git diff
    --check`.

- [x] `OZ-COMP-001S` Remote device mutation gateway dispatch
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: explicit `--url` on `openzues devices remove`, `clear`,
    `reject`, `rotate`, and `revoke` dispatches to the fakeable remote gateway
    method call, forwards auth/timeout options, preserves JSON output, and
    does not leak secret values.
  - Evidence required: focused remote devices mutation proof, adjacent
    devices/pairing CLI proof, ruff, mypy
  - Status: checkpointed in `c54866d9`
  - Weight: 1
  - Last verified: 2026-05-12,
    `python -m pytest tests\test_cli.py::test_devices_remote_mutation_commands_call_remote_gateway_with_auth_flags tests\test_cli.py::test_devices_clear_remote_gateway_removes_and_rejects_with_auth_flags -q`
    (`5 failed` before implementation with Typer exit 2, then `5 passed`),
    adjacent
    `python -m pytest tests\test_cli.py -q -k "devices_ or pairing_approve"`
    (`21 passed, 596 deselected`), `ruff check src\openzues\cli.py
    tests\test_cli.py`, `mypy src\openzues\cli.py`, and focused `git diff
    --check`.

- [x] `OZ-COMP-001T` Configured remote device CLI defaults
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: device CLI commands resolve saved `gateway.remote.url` and remote
    auth defaults when explicit URL/auth flags are omitted, normalize host-only
    remote URLs, keep CLI flags higher precedence, and preserve local dispatch
    when no configured remote URL exists.
  - Evidence required: focused configured remote devices proof, adjacent
    devices/pairing CLI proof, ruff, mypy
  - Status: checkpointed in `aa2cbf50`
  - Weight: 1
  - Last verified: 2026-05-12,
    `python -m pytest tests\test_cli.py::test_devices_list_uses_configured_remote_gateway_when_url_omitted -q`
    (`1 failed` before implementation because dispatch stayed local, then
    `1 passed`), adjacent
    `python -m pytest tests\test_cli.py -q -k "devices_ or pairing_approve"`
    (`22 passed, 596 deselected`), `ruff check src\openzues\cli.py
    tests\test_cli.py`, `mypy src\openzues\cli.py`, and focused `git diff
    --check`.

- [x] `OZ-COMP-001U` Loopback device CLI local fallback
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: configured loopback remote device dispatch retries through the
    local pairing service for pairing-required / pending-approval failures,
    while explicit `--url` and non-loopback remotes remain remote-strict.
  - Evidence required: focused loopback fallback devices proof, adjacent
    devices/pairing CLI proof, ruff, mypy
  - Status: checkpointed in `3616f48b`
  - Weight: 1
  - Last verified: 2026-05-12,
    `python -m pytest tests\test_cli.py::test_devices_list_falls_back_to_local_pairing_on_configured_loopback_rejection -q`
    (`1 failed` before implementation with remote `RuntimeError`, then
    `1 passed`), adjacent
    `python -m pytest tests\test_cli.py -q -k "devices_ or pairing_approve"`
    (`23 passed, 596 deselected`), `ruff check src\openzues\cli.py
    tests\test_cli.py`, `mypy src\openzues\cli.py`, and focused `git diff
    --check`.

- [x] `OZ-COMP-001V` Device approve preview approval-state metadata
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`,
    `openclaw-main/src/shared/device-pairing-access.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues devices approve --latest --json` includes
    `approvalState.kind`, requested access, and approved access summaries with
    upstream role/scope normalization and paired-device classification.
  - Evidence required: focused approval-state preview proof, adjacent
    devices/pairing CLI proof, ruff, mypy
  - Status: checkpointed in `a45702fc`
  - Weight: 1
  - Last verified: 2026-05-12,
    `python -m pytest tests\test_cli.py::test_devices_approve_latest_json_includes_approval_state -q`
    (`1 failed` before implementation with missing `approvalState`, then
    `1 passed`), adjacent
    `python -m pytest tests\test_cli.py -q -k "devices_ or pairing_approve"`
    (`24 passed, 596 deselected`), `ruff check src\openzues\cli.py
    tests\test_cli.py`, `mypy src\openzues\cli.py`, and focused `git diff
    --check`.
