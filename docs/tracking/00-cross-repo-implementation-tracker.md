...
| --- | ---: | --- | --- |
| Repo-wide OpenClaw parity in OpenZues | ~92.7% | Active, broad parity still open | `docs/openclaw-parity-progress.md`, `docs/openclaw-parity-unresolved-seams.md` |
...
The imported plugin SDK request-url slice is reverified from the existing
`f4a23a25` fetch/SSRF source-test checkpoint.
...
| OZ-PLUGIN-001 | Real installed plugin module import/activation | Request-url helper reverified from `f4a23a25` and tracker refreshed | Repo-wide +0.1%, plugin metadata/runtime +0.1% | Continue broader unresolved helper/runtime queue; next `persistent-dedupe` |
