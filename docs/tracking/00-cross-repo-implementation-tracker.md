...
| --- | ---: | --- | --- |
| Repo-wide OpenClaw parity in OpenZues | ~99.9% | Active, broad parity still open; evidence band ~80-99.99999999999999999999999999999999999999999999999995% | `docs/openclaw-parity-progress.md`, `docs/openclaw-parity-unresolved-seams.md` |
...
| Runtime/CLI/doctor native bridge | ~99.93% | Mostly landed; packaging, QR non-env/gateway-backed remote secret resolution, and installed plugin depth remain | `docs/openclaw-parity-progress.md` |
...
| OZ-COMP-001 | Companion apps/nodes parity | QR approval guidance checkpointed in `d6052fda`; QR JSON setup-code contract checkpointed in `b79b87c3`; device pairing public-key persistence/projection checkpointed in `bef0652f`; QR public-cleartext URL rejection checkpointed in `24d89215`; QR remote saved URL/auth config checkpointed in `3d63d136`; QR Tailscale MagicDNS checkpointed in `beb67302`; QR remote env SecretRef diagnostics checkpointed in `e3b3031a` | Repo-wide +0.1%, companion/setup breadth +0.1% | Continue QR non-env/gateway-backed remote secret resolution or next startup/update edge |
...
source/test checkpointed in `e3b3031a`. Continue non-env/gateway-backed QR
remote secret resolution, provider/runtime breadth, or the next package
startup/update edge.
