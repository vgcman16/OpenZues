...
| Packaging/companion app breadth | ~6.4% | Low, broad parity still open | QR setup-code safety, local/remote SecretRef slices, device pairing CLI mutations, approve-preview gateway flag preservation, remote device command dispatch, configured remote defaults, loopback fallback, approval-state preview metadata, device token scope-preserving rotation, approved-role gates, approved-scope baselines, and requested-scope approval gates are landed; companion apps remain mostly open |

Latest verified adjustment: `OZ-COMP-001AA` requested operator-scope pairing
approval gates move packaging/companion breadth to ~6.4%; repo-wide OpenClaw
