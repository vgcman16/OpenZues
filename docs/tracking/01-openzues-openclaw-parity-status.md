...
- [x] ACP `resumeSessionId` ownership enforcement for `sessions.spawn`.
  ...
  - Contract: resume ids must match persisted ACP runtime identity metadata
    owned by the requester session; foreign ids return `resume_forbidden`
    before runtime dispatch, while owned ids continue to dispatch.
  - Evidence required: focused ACP resume proof, adjacent ACP gateway proof,
    ACP runtime service proof, ruff, mypy
  - Status: checkpointed in `27b7e14e`
  - Weight: 1
...
- [x] ACP model/thinking override propagation for `sessions.spawn`.
  ...
  - Target: `src/openzues/services/gateway_node_methods.py`,
    ...
    `src/openzues/services/codex_rpc.py`, `tests/test_gateway_node_methods.py`,
    `tests/test_gateway_acp_spawn.py`, `tests/test_codex_rpc.py`
  - Contract: `model` and `thinking` ACP spawn params are forwarded to runtime
    startup/turn dispatch, persisted on spawned session metadata, and
    serialized as Codex app-server model/effort fields.
  - Evidence required: focused gateway/runtime/RPC proofs, adjacent ACP/RPC
    tests, ruff, mypy
  - Status: checkpointed in `c10e37f1`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green gateway/runtime/RPC tests,
    adjacent ACP runtime proof (`20 passed`), adjacent gateway ACP proof (`9
    passed, 1265 deselected`), RPC proof (`9 passed`), ruff, mypy, and focused
    `git diff --check`.

- [x] ACP configured runtime-agent alias mapping for `sessions.spawn`.
  - Source: `openclaw-main/src/agents/acp-spawn.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_node_methods.py`,
    `src/openzues/schemas.py`, `tests/test_gateway_node_methods.py`
  - Contract: a configured OpenZues agent whose `runtime.type` is `acp` maps to
    the configured ACP harness id before allowlist checks and runtime dispatch,
    while spawned-session metadata records the resolved runtime agent id.
  - Evidence required: focused ACP alias proof, adjacent ACP gateway proof,
    ruff, mypy
  - Status: checkpointed in `ccbbd403`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green
    `python -m pytest tests\test_gateway_node_methods.py::test_sessions_spawn_acp_maps_configured_runtime_agent_alias -q`
    (`1 passed` after implementation), adjacent ACP gateway proof (`4 passed,
    1271 deselected`), ruff, mypy, and focused `git diff --check`.

- [x] ACP native-agent mismatch preflight for `sessions.spawn`.
  - Source: `openclaw-main/src/agents/acp-spawn.ts`,
    `openclaw-main/src/agents/acp-spawn.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_node_methods.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: a configured native OpenZues agent id requested with
    `runtime="acp"` returns `runtime_agent_mismatch` before dispatch unless it
    is explicitly an allowed ACP harness id or declares `runtime.type="acp"`.
  - Evidence required: focused ACP mismatch proof, adjacent ACP gateway proof,
    ruff, mypy
  - Status: checkpointed in `a0cccb08`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green
    `python -m pytest tests\test_gateway_node_methods.py::test_sessions_spawn_acp_rejects_configured_native_agent_id -q`
    (failed before implementation with `agent_forbidden`), paired ACP proof
    (`2 passed`), adjacent ACP gateway proof (`5 passed, 1271 deselected`),
    ruff, mypy, and focused `git diff --check`.

- [x] ACP run-timeout runtime propagation for `sessions.spawn`.
  - Source: `openclaw-main/src/agents/acp-spawn.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_acp_spawn.py`,
    `src/openzues/services/manager.py`, `src/openzues/services/codex_rpc.py`,
    `tests/test_gateway_acp_spawn.py`, `tests/test_codex_rpc.py`
  - Contract: `runTimeoutSeconds` reaches ACP runtime session initialization
    and turn dispatch, then applies to Codex app-server `thread/start` and
    `turn/start` request timeouts.
  - Evidence required: focused ACP runtime proof, RPC proof, adjacent ACP/RPC
    suites, ruff, mypy
  - Status: checkpointed in `3e821817`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green ACP runtime proof (`1
    passed` after implementation), focused RPC timeout proof (`2 passed`),
    adjacent ACP runtime suite (`21 passed`), RPC suite (`11 passed`), gateway
    timeout tracking proof (`1 passed`), ruff, mypy, and focused
    `git diff --check`.

- [x] ACP subagent depth and child-cap policy for `sessions.spawn`.
  - Source: `openclaw-main/src/agents/acp-spawn.test.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_node_methods.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: ACP spawns obey configured `maxSpawnDepth` and
    `maxChildrenPerAgent` before runtime dispatch, returning OpenClaw-shaped
    `subagent_policy` errors.
  - Evidence required: focused ACP policy proof, adjacent ACP gateway proof,
    ruff, mypy
  - Status: checkpointed in `1bd16f72`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green ACP depth/child-cap proof
    (`2 passed` after implementation), adjacent ACP gateway proof (`6 passed,
    1272 deselected`), ruff, mypy, and focused `git diff --check`.

- [x] ACP subagent target allowlist policy for `sessions.spawn`.
  - Source: `openclaw-main/src/agents/acp-spawn.test.ts`,
    `openclaw-main/src/agents/subagent-target-policy.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_node_methods.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: child ACP spawns obey requester-specific
    `subagents.allowAgents`; explicit targets outside the allowlist and
    explicit self-targets excluded by the allowlist return `subagent_policy`
    before ACP runtime dispatch, while top-level ACP spawns preserve existing
    ACP allowlist/runtime-agent checks.
  - Evidence required: focused ACP allowlist proof, adjacent ACP gateway proof,
    ruff, mypy
  - Status: checkpointed in `39d3b06b`
  - Weight: 1
...
- [x] Browser/canvas/nodes/voice bounded bridge, including native browser
  ...
  configured-node selection/local runtime fallback,
...
- [x] `browser.request` native node-proxy dispatch.
  - Source: `openclaw-main/extensions/browser/src/gateway/browser-request.ts`
  - Target: `src/openzues/services/gateway_node_methods.py`
  - Contract: validate `method`/`path`, allow GET/POST/DELETE only, reject
    persistent profile mutations, invoke the connected browser-capable node's
    `browser.proxy` command with query/body/timeout/profile metadata, and
    return the upstream-shaped `result`.
  - Status: checkpointed in `430e365b`
  - Last verified: 2026-05-12, focused red/green browser-request tests,
    adjacent node/browser proof (`6 passed, 1235 deselected`), ruff, and mypy.

- [x] `browser.request` proxy-file persistence and path rewriting.
  - Source: `openclaw-main/extensions/browser/src/browser/proxy-files.ts`
  - Target: `src/openzues/services/gateway_node_methods.py`
  - Contract: accept upstream-shaped `files[]` from `browser.proxy`, decode
    bounded base64 payloads, store them in the native browser proxy media
    directory, and rewrite `result.path`, `result.imagePath`, and
...
- [x] `browser.request` local browser-runtime fallback.
  - Source: `openclaw-main/extensions/browser/src/gateway/browser-request.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_node_methods.py`,
    `src/openzues/services/gateway_browser_runtime.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: when no browser-capable node is selected, forward
    method/path/query/body/timeout into a fakeable native browser request
    adapter, preserve the upstream disabled/runtime unavailable error shape,
    and map route-style status/body responses to gateway method success/errors.
  - Evidence required: focused local-fallback proof, adjacent browser/node
    proof, ruff, mypy
  - Status: checkpointed in `45fd0ffd`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green local-fallback test, adjacent
    browser/node proof (`9 passed, 1236 deselected`), ruff, mypy, and focused
    `git diff --check`.

- [x] `browser.request` local lifecycle routes.
  ...
  - Target: `src/openzues/services/gateway_browser_runtime.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: native local request dispatch maps `POST /start` and
    `POST /stop` into the existing browser lifecycle runtime, preserving
    profile/session routing and `all` / `allSessions` stop options.
  - Evidence required: focused lifecycle route proof, adjacent browser
    lifecycle/request proof, ruff, mypy
  - Status: checkpointed in `d9b5e87d`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green lifecycle route test,
    adjacent browser proof (`8 passed, 1238 deselected`), ruff, mypy, and
    focused `git diff --check`.

- [x] `browser.request` local tab mutation routes.
  - Source: `openclaw-main/extensions/browser/src/browser/routes/tabs.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_browser_runtime.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: native local request dispatch maps `POST /tabs/focus` and
    `DELETE /tabs/:targetId` into existing browser focus/targeted-close
    runtime methods, including decoded path-segment target ids.
  - Evidence required: focused tab mutation route proof, adjacent browser
    request/tab proof, ruff, mypy
  - Status: checkpointed in `aa7298c6`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green tab mutation route test,
    adjacent browser proof (`12 passed, 1235 deselected`), ruff, mypy, and
    focused `git diff --check`.

- [x] `browser.request` local tab action close/select routes.
  - Source: `openclaw-main/extensions/browser/src/browser/routes/tabs.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_browser_runtime.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: native local request dispatch maps `/tabs/action` `close` and
    `select` actions by resolving the requested tab index from the native tab
    list, then dispatching targeted close or focus.
  - Evidence required: focused tab-action route proof, adjacent browser
    request/tab proof, ruff, mypy
  - Status: checkpointed in `904f2673`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green tab-action route test,
    adjacent browser proof (`13 passed, 1235 deselected`), ruff, mypy, and
    focused `git diff --check`.

- [x] `browser.request` local storage routes.
  - Source: `openclaw-main/extensions/browser/src/browser/routes/agent.storage.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_browser_runtime.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: native local request dispatch maps `GET /storage/:kind`,
    `POST /storage/:kind/set`, and `POST /storage/:kind/clear` into existing
    local/session browser storage get/set/clear runtime methods.
  - Evidence required: focused storage route proof, adjacent browser
    request/storage proof, ruff, mypy
  - Status: checkpointed in `483e4aff`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green storage route test, adjacent
    browser proof (`13 passed, 1236 deselected`), ruff, mypy, and focused
    `git diff --check`.

- [x] `browser.request` local cookie routes.
  - Source: `openclaw-main/extensions/browser/src/browser/routes/agent.storage.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_browser_runtime.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: native local request dispatch maps `GET /cookies`,
    `POST /cookies/set`, and `POST /cookies/clear` into existing browser cookie
    get/set/clear runtime methods, including nested upstream `cookie` set
    fields.
  - Evidence required: focused cookie route proof, adjacent browser
    request/cookie/storage proof, ruff, mypy
  - Status: checkpointed in `97602084`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green cookie route test, adjacent
    browser proof (`16 passed, 1234 deselected`), ruff, mypy, and focused
    `git diff --check`.

- [x] `browser.request` local debug routes.
  - Source: `openclaw-main/extensions/browser/src/browser/routes/agent.debug.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_browser_runtime.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: native local request dispatch maps `GET /console`,
    `GET /errors`, `GET /requests`, `POST /trace/start`, and
    `POST /trace/stop` into existing console, page-error, network-request, and
    trace artifact runtime methods, preserving supported clear/filter flags
    and controlled trace output paths.
  - Evidence required: focused debug route proof, adjacent browser
    request/debug/network/trace proof, ruff, mypy
  - Status: checkpointed in `d7eb616d`
  - Weight: 1
...
- [x] `browser.request` local act utility routes.
  - Source: `openclaw-main/extensions/browser/src/browser/routes/agent.act.ts`,
    ...
    `openclaw-main/extensions/browser/src/browser/routes/agent.act.download.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/services/gateway_browser_runtime.py`,
    `tests/test_gateway_node_methods.py`
  - Contract: native local request dispatch maps `POST /highlight`,
    `POST /download`, and `POST /hooks/file-chooser` onto existing guarded
    highlight, download, and upload runtime methods with controlled artifact
    paths.
  - Evidence required: focused act utility route proof, adjacent browser
    request/action/download/upload/debug proof, ruff, mypy
  - Status: checkpointed in `886c86f3`
  - Weight: 1
  - Last verified: 2026-05-12, focused red/green act utility route test,
...
- [x] Device pairing CLI for QR companion flow: `openzues devices list` and
  `openzues devices approve`, including JSON output and preview-only latest
  approval selection.
  - Status: checkpointed in `25c344be`

- [x] Device pairing CLI mutation surface: `openzues devices remove`,
  `clear`, `reject`, `rotate`, and `revoke`, including JSON output, guarded
  clear, optional pending rejection, and repeated rotation scopes.
  ...
  - Last verified: 2026-05-12, focused mutation CLI proof (`5 passed`),
    adjacent devices/pairing CLI proof (`13 passed, 596 deselected`), `ruff
    check`, `mypy`, and focused `git diff --check`.

- [x] Device approve preview gateway flags: `openzues devices approve
  --latest` preserves `--url`/`--timeout` in the rerun command and represents
  `--token`/`--password` only as `requiresAuthFlags` booleans.
  - Status: checkpointed in `204bcdd2`
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Last verified: 2026-05-12, focused preview proof (`2 passed`),
    adjacent devices/pairing CLI proof (`14 passed, 596 deselected`), `ruff
    check`, `mypy`, and focused `git diff --check`.

- [x] Remote device list/approve gateway dispatch: explicit `--url` on
  `openzues devices list` and `openzues devices approve <requestId>` forwards
  auth and timeout options to a fakeable remote gateway method call.
  - Status: checkpointed in `fa6d230a`
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Last verified: 2026-05-12, focused remote dispatch proof (`3 passed`),
    adjacent devices/pairing CLI proof (`16 passed, 596 deselected`), `ruff
    check`, `mypy`, and focused `git diff --check`.

- [x] Remote device mutation gateway dispatch: explicit `--url` on
  `openzues devices remove`, `clear`, `reject`, `rotate`, and `revoke`
  forwards auth and timeout options to the same fakeable remote gateway method
  call.
  - Status: checkpointed in `c54866d9`
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Last verified: 2026-05-12, focused remote mutation proof (`5 passed`),
    adjacent devices/pairing CLI proof (`21 passed, 596 deselected`), `ruff
    check`, `mypy`, and focused `git diff --check`.

- [x] Configured remote device CLI defaults: saved `gateway.remote.url` and
  remote auth are used when explicit device CLI URL/auth flags are omitted.
  - Status: checkpointed in `aa2cbf50`
  - Source: `openclaw-main/src/cli/devices-cli.ts`,
    `openclaw-main/src/cli/devices-cli.test.ts`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Last verified: 2026-05-12, focused configured-remote proof (`1 passed`),
    adjacent devices/pairing CLI proof (`22 passed, 596 deselected`), `ruff
    check`, `mypy`, and focused `git diff --check`.

- [x] Loopback device CLI local fallback: configured loopback remotes retry
  local pairing after pairing-required or pending-approval failures.
...
- [x] Package update downgrade confirmation: dry-run downgrade risk, JSON/non-
  interactive downgrade blocking, and `--yes`-confirmed package dispatch.
  - Status: checkpointed in `e39ead6e`

- [x] Stored-channel package update dispatch, using saved `update.channel`
  when `openzues update` is run without `--channel`.
  - Status: checkpointed in `688b89c3`

- [x] Package update Node engine preflight for registry package targets,
  blocking incompatible targets before package-manager dispatch.
  - Status: checkpointed in `6897eae6`

- [x] Package update activated plugin restart-health failure, projecting
...
- [x] Package update gateway-version restart-health failure, projecting
...
- [x] All-shell native completion write-state cache generation for
  `openzues.bash`, `openzues.fish`, `openzues.ps1`, and `openzues.zsh`.
...
- [x] Route-backed Telegram HTTP-error stale-thread fallback for media sends
  and polls.
  - Status: checkpointed in `0544ceb2`; poll proof in `1eadc4ae`

- [x] LINE signed webhook ingress for `/line/webhook`, including raw-body
  HMAC-SHA256 signature validation and native OpsMesh dispatch.
  - Status: checkpointed in `c3279e34`

- [x] LINE text webhook session delivery for direct user message events,
  including LINE conversation-target resolution, session-key derivation,
  session-backed text delivery, redacted reply-token metadata, and
  sender/conversation/delivery result projection.
  - Status: checkpointed in `906a7147`

- [x] LINE postback webhook session delivery for direct user postback events,
  including raw postback delivery and OpenClaw-shaped `line.action` /
  `line.device` normalization before session dispatch.
  - Status: checkpointed in `b1bab063`

- [x] LINE media placeholder webhook session delivery for image/video/audio/file
  message events.
  - Status: checkpointed in `e594dfeb`

- [x] LINE sticker webhook session delivery, including package-name lookup and
  keyword/text descriptor projection.
  - Status: checkpointed in `583bd41a`

- [x] LINE location webhook session delivery, including map-pin coordinate text
  with fixed six-decimal latitude/longitude.
  - Status: checkpointed in `a3a4ad91`

- [x] LINE default group mention gating before session delivery.
  - Status: checkpointed in `beeb2d4a`

- [x] LINE native mention metadata handling for group session delivery.
  - Status: checkpointed in `24c9f902`

- [x] LINE group pending-history replay into mentioned session delivery.
  - Status: checkpointed in `8d021ced`

- [x] LINE non-text group media mention-gate bypass.
...
- [x] LINE webhook redelivery replay dedupe.
  ...
  - Contract: message redeliveries dedupe by LINE message id even when
    `webhookEventId` changes; postback/non-message redeliveries dedupe by
    account, event type, source id, and `webhookEventId`; duplicates return
    skip metadata and do not re-enter native session delivery.
  - Evidence required: focused LINE redelivery proofs, adjacent LINE
    webhook/provider proof, ruff, mypy
  - Status: checkpointed in `9acc4cd6`
  - Weight: 1
...
- [x] Provider-gated plugin native command specs for gateway `commands.list`,
  preserving OpenClaw's distinction between ungated provider alias helpers and
  gated provider-native command publication.
...
- [x] Companion QR remote env SecretRef diagnostics, preserving OpenClaw's
  `[secrets]` diagnostics routing for env-backed
  `gateway.remote.token/password` resolution in JSON/setup-code output.
...
- [x] Owning npm-root manager detection, preserving OpenClaw's ability to infer
...
- [x] Git update post-doctor control-UI repair, preserving source-checkout
  UI build, native doctor-fix, post-doctor asset repair, and
  `ui-assets-missing` failure projection for missing required web assets.
...
- [x] Discord provider-native video-caption split delivery with OpenClaw-shaped
...
- [ ] Runtime-control hard gaps.
  ...
  - Status: open; transcript artifact methods checkpointed in `13eddac7`,
    ...
    `nativeHook.invoke` checkpointed in `587c181b`, and `push.web.*`
    checkpointed in `af0fc977`; OpenClaw gateway `server-methods/*.ts`
    handler sweep currently has no missing method names
  - Weight: 3
...
- [x] `OZ-RT-001BG` Gateway web-push methods.
  ...
  - Weight: 1
  - Last verified: 2026-05-12, focused web-push gateway proofs (`2 passed`),
    adjacent `push_web or push_test` proof (`6 passed, 1263 deselected`),
    policy proof (`2 passed, 17 deselected`), handler-string sweep with no
    missing OpenClaw `server-methods/*.ts` method names, `ruff check`, `mypy`,
    and focused `git diff --check`.
...
- [x] Owning npm-root manager detection.
  - Source: `openclaw-main/src/infra/update-global.ts`,
    `openclaw-main/src/infra/update-global.test.ts`,
  ...
  - Status: checkpointed in `76e2a21c`.
  - Weight: 1
  - Last verified: 2026-05-08, focused CLI red/green proof (`1 failed` before
    implementation, then `1 passed`), focused CLI/runtime proof (`2 passed`),
    adjacent CLI package proof (`5 passed, 558 deselected`), adjacent
...
- [x] QQBot structured self-closing media tags.
  - Source: `openclaw-main/extensions/qqbot/src/utils/media-tags.ts`,
    `openclaw-main/extensions/qqbot/src/utils/media-tags.test.ts`,
    `openclaw-main/extensions/qqbot/src/engine/messaging/outbound.ts`,
    `openclaw-main/extensions/qqbot/src/engine/messaging/outbound-media-send.ts`
  - References: Hermes/Warp paths or `none`
  - Target: `src/openzues/services/ops_mesh.py`
  - Contract: route-backed QQBot sends normalize entity-encoded and
    self-closing media tags, resolve upstream aliases into canonical QQBot tag
    kinds, extract `file`/`src`/`path`/`url` attribute media sources, and route
    generic `qqmedia` through file/video/voice/image auto-detection before
    provider upload.
  - Evidence required: focused self-closing media-tag test, focused adjacent
    QQBot provider proof, adjacent provider proof, ruff, mypy
  - Status: checkpointed in `b5a821a7`
  - Weight: 1
  - Last verified: 2026-05-12,
    ...
    sent as text without `mediaIds`, then `1 passed`), focused adjacent
    `python -m pytest tests\test_ops_mesh.py -q -k "qqbot and (self_closing or image_tag or image_media or reply_seq or local_media or native_route)"`
    (`6 passed, 497 deselected`), adjacent
    `python -m pytest tests\test_ops_mesh.py -q -k "qqbot or zalo_native_route or provider_native_options"`
    (`11 passed, 492 deselected`), `ruff check
    src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, `mypy
    src\openzues\services\ops_mesh.py`, and focused `git diff --check`.

- [x] QQBot chunked local media uploads.
  - Source: `openclaw-main/extensions/qqbot/src/engine/api/media-chunked.ts`,
    `openclaw-main/extensions/qqbot/src/engine/api/media-chunked.test.ts`,
    `openclaw-main/extensions/qqbot/src/engine/messaging/sender.ts`,
    `openclaw-main/extensions/qqbot/src/utils/file-utils.ts`
  - References: Hermes/Warp paths or `none`
  - Target: `src/openzues/services/ops_mesh.py`
  - Contract: large allowed local C2C/group media sends at or above the
    OpenClaw 5 MiB threshold use `upload_prepare`, presigned part PUTs,
    `upload_part_finish`, and `upload_id` completion before the normal
    `file_info` media message path.
  - Evidence required: focused chunked local media test, focused adjacent
    QQBot provider proof, adjacent provider proof, ruff, mypy
  - Status: checkpointed in `ee990c4f`
  - Weight: 1
  - Last verified: 2026-05-12,
    ...
    (`1 failed` before implementation because the local file stayed on the
    one-shot `/files` upload path without `file_info`, then `1 passed`),
    focused adjacent `python -m pytest tests\test_ops_mesh.py -q -k "qqbot and (chunked or self_closing or image_tag or image_media or reply_seq or local_media or native_route)"`
    (`7 passed, 497 deselected`), adjacent
    `python -m pytest tests\test_ops_mesh.py -q -k "qqbot or zalo_native_route or provider_native_options"`
    (`12 passed, 492 deselected`), `ruff check
    src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, `mypy
    src\openzues\services\ops_mesh.py`, and focused `git diff --check`.

- [x] QQBot voice-to-file fallback.
  - Source: `openclaw-main/extensions/qqbot/src/engine/messaging/outbound.ts`,
    `openclaw-main/extensions/qqbot/src/engine/messaging/outbound-media-send.ts`
  - References: Hermes/Warp paths or `none`
  - Target: `src/openzues/services/ops_mesh.py`
  - Contract: voice media sends first try QQBot voice upload, then retry the
    same source as file/document upload when the provider rejects the voice
    path; successful fallback returns normal `file_info` delivery and provider
    metadata reports `mediaType=file`.
  - Evidence required: focused fallback test, focused adjacent QQBot provider
    proof, adjacent provider proof, ruff, mypy
  - Status: checkpointed in `0ccc41c0`
  - Weight: 1
  - Last verified: 2026-05-12,
    ...
    (`1 failed` before implementation because the voice upload error surfaced
    without retrying as a file, then `1 passed`), focused adjacent
    `python -m pytest tests\test_ops_mesh.py -q -k "qqbot and (falls_back or chunked or self_closing or image_tag or image_media or reply_seq or local_media or native_route)"`
    (`8 passed, 497 deselected`), adjacent
    `python -m pytest tests\test_ops_mesh.py -q -k "qqbot or zalo_native_route or provider_native_options"`
    (`13 passed, 492 deselected`), `ruff check
    src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, `mypy
    src\openzues\services\ops_mesh.py`, and focused `git diff --check`.

- [x] QQBot file-media text follow-up delivery.
  - Source: `openclaw-main/extensions/qqbot/src/engine/messaging/outbound.ts`
  - References: Hermes/Warp paths or `none`
  - Target: `src/openzues/services/ops_mesh.py`
  - Contract: file/document media sends post the media message first, then
    send accompanying text through the normal QQBot text path without replacing
    the media result's `messageId`/`messageIds`.
  - Evidence required: focused text-after-media test, focused adjacent QQBot
    provider proof, adjacent provider proof, ruff, mypy
  - Status: checkpointed in `ea9dfe30`
  - Weight: 1
  - Last verified: 2026-05-12,
    ...
    (`1 failed` before implementation because only the media message was
    posted, then `1 passed`), focused adjacent
    ...
    (`9 passed, 497 deselected`), adjacent
    `python -m pytest tests\test_ops_mesh.py -q -k "qqbot or zalo_native_route or provider_native_options"`
    (`14 passed, 492 deselected`), `ruff check
    src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, `mypy
    src\openzues\services\ops_mesh.py`, and focused `git diff --check`.

- [x] QQBot direct image/video media text follow-up delivery.
  - Source: `openclaw-main/extensions/qqbot/src/engine/messaging/outbound.ts`,
    `openclaw-main/extensions/qqbot/src/engine/messaging/sender.ts`
  - References: Hermes/Warp paths or `none`
  - Target: `src/openzues/services/ops_mesh.py`
  - Contract: direct image/video media sends keep the media payload free of
    caption text, preserve the media result as the delivery result, and send
    accompanying text through the normal QQBot text path after media delivery.
  - Evidence required: focused image text-after-media test, focused adjacent
    QQBot provider proof, adjacent provider proof, ruff, mypy
  - Status: checkpointed in `0ea67d9f`
  - Weight: 1
  - Last verified: 2026-05-12,
    `python -m pytest tests\test_ops_mesh.py::test_ops_mesh_service_send_direct_channel_message_uploads_qqbot_image_media -q`
    (`1 failed` before implementation because the media payload still carried
    `content` and no follow-up text was posted, then `1 passed`), focused
    adjacent
...
- [x] QQBot inline media text ordering.
  - Source: `openclaw-main/extensions/qqbot/src/engine/messaging/outbound.ts`
  - References: Hermes/Warp paths or `none`
  - Target: `src/openzues/services/ops_mesh.py`
  - Contract: route-backed inline media tags are parsed into ordered text/media
    segments; text fragments are sent through the QQBot text path before or
    after adjacent media, and media payloads do not embed those fragments as
    `content`.
  - Evidence required: focused inline image-tag ordering test, focused
    adjacent QQBot provider proof, adjacent provider proof, ruff, mypy
  - Status: checkpointed in `9b34e2aa`
  - Weight: 1
  - Last verified: 2026-05-12,
    `python -m pytest tests\test_ops_mesh.py::test_ops_mesh_service_send_direct_channel_message_extracts_qqbot_image_tag -q`
    (`1 failed` before implementation because inline text was embedded in the
    media payload instead of sent first, then `1 passed`), focused adjacent
...
- [x] QQBot inline trailing result metadata.
  - Source: `openclaw-main/extensions/qqbot/src/engine/messaging/outbound.ts`
  - References: Hermes/Warp paths or `none`
  - Target: `src/openzues/services/ops_mesh.py`
  - Contract: when an inline media queue ends with text after media, the
    provider `messageId` reports that trailing text message id, matching
    OpenClaw's `lastResult`, while media ids and media message ids remain
    available in the result metadata.
  - Evidence required: focused inline trailing-result test, focused adjacent
    QQBot provider proof, adjacent provider proof, ruff, mypy
  - Status: checkpointed in `34a31061`
  - Weight: 1
  - Last verified: 2026-05-12,
    ...
    media message id, then `1 passed`), focused adjacent
    ...
    (`10 passed, 497 deselected`), adjacent
    `python -m pytest tests\test_ops_mesh.py -q -k "qqbot or zalo_native_route or provider_native_options"`
...
- [x] QR remote env SecretRef diagnostics.
  - Source: `openclaw-main/src/cli/qr-cli.ts`,
    `openclaw-main/src/cli/qr-cli.test.ts`,
  ...
  - Contract: `openzues qr --remote` resolves env-backed
    ...
    setup-code-only output, and keeps resolved secret values out of stdout and
    diagnostics.
  ...
  - Last verified: 2026-05-08, focused red/green
    ...
    (`1 failed` before implementation, then `1 passed`), adjacent QR CLI proof
    (`10 passed, 558 deselected`), `ruff check src\openzues\cli.py
    tests\test_cli.py`, `mypy src\openzues\cli.py`, and focused
    `git diff --check`.

- [x] QR unresolved remote SecretRef auth preflight.
  - Source: `openclaw-main/src/cli/qr-cli.ts`,
    `openclaw-main/src/pairing/setup-code.ts`,
    `openclaw-main/src/cli/qr-cli.test.ts`
  - References: Hermes/Warp `none`
  ...
  - Contract: `openzues qr --remote` emits unresolved remote SecretRef
    ...
    gateway auth exists, and does not issue a device bootstrap token.
...
- [x] QR remote exec SecretRef resolution.
  - Source: `openclaw-main/src/secrets/resolve.ts`,
    `openclaw-main/src/secrets/resolve.test.ts`,
    `openclaw-main/src/cli/qr-cli.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/cli.py`, `tests/test_cli.py`
  - Contract: `openzues qr --remote` resolves exec-backed remote auth
    SecretRefs through persisted `secrets.providers`, sends the OpenClaw
    protocol v1 request envelope to the configured command, reads protocol v1
    `values`, routes diagnostics to stderr for JSON output, and keeps resolved
...
- [x] Direct outbound `channelData`-only payloads.
  ...
  - Contract: provider-native direct `send` treats non-empty `channelData` as
    sendable content with empty text, forwards structured `channel_data` to
    the shared outbound runtime, and persists `channelData` on the delivery
    row.
...
- [x] Slack reaction event system wakes.
...
- [x] Slack pin event system wakes.
  - Source: `openclaw-main/extensions/slack/src/monitor/events.ts`,
...
- [x] Slack modal lifecycle interactions.
  - Source:
    `openclaw-main/extensions/slack/src/monitor/events/interactions.ts`,
...
- [x] Slack signing env SecretRefs.
...
- [x] Slack signing exec SecretRefs.
  - Source: `openclaw-main/extensions/slack/src/config-schema.test.ts`,
    `openclaw-main/src/config/types.secrets.ts`
  - References: Hermes/Warp `none`
  - Target: `src/openzues/app.py`, `tests/test_ops_mesh.py`
...
- [x] Slack external arg-menu empty options.
  - Source: `openclaw-main/extensions/slack/src/monitor/slash.ts`,
...
- [x] WhatsApp split-media reusable reply fanout.
  ...
  - Contract: route-backed provider requests preserve `replyToIdSource` and
    ...
    each media payload for reusable explicit/all-mode replies.
...
- [x] Telegram multi-media reply fanout.
  - Source: `openclaw-main/src/infra/outbound/reply-policy.ts`,
    `openclaw-main/src/infra/outbound/message-plan.test.ts`,
