...
| Active gateway/session/tool-contract family | ~99.958% | High for bounded local path | Does not mean whole product parity |
...
| Browser/canvas/nodes/voice bounded command family | ~99.95% | High for bounded local path | No longer active queue head |
