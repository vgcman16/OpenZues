...
| Packaging/companion app breadth | ~6.2% | Low, broad parity still open | QR setup-code safety, local/remote SecretRef slices, device pairing CLI mutations, approve-preview gateway flag preservation, remote device command dispatch, configured remote defaults, loopback fallback, approval-state preview metadata, device token scope-preserving rotation, and approved-role device token gates are landed; companion apps remain mostly open |

Latest verified adjustment: `OZ-COMP-001Y` device-token approved-role gates
move packaging/companion breadth to ~6.2%; repo-wide OpenClaw parity remains
estimated at ~99.9%.
