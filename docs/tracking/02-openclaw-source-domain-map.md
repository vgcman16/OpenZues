...
| [~] | Plugin and extension system | SDK, manifests, bundled/installed plugins, lifecycle, hooks, ClawHub/npm packaging; document and web-content extractor contracts plus CommonJS/ESM runtime import/execution, request-time tool factory context, text-runtime string helpers, error-runtime helpers, temp-path helpers, secret-input helpers, routing/session helper shims, and reply-chunking helpers verified; broader SDK helper/runtime contracts remain | `src/plugins`, `src/plugin-sdk`, `extensions`, `packages/plugin-sdk` |
