...
| [~] | Media, voice, web, canvas | canvas shortcode normalization verified; image/video/realtime voice helper shims landed; music generation, media understanding, web search/fetch, Canvas/A2UI breadth remain | `src/media`, `src/image-generation`, `src/realtime-voice`, `src/canvas-host`, `extensions/comfy` |
