...
| [~] | Media, voice, web, canvas | canvas shortcode normalization verified; image/video/realtime voice/media understanding runtime helper shims landed; music generation, media understanding provider helpers, web search/fetch, Canvas/A2UI breadth remain | `src/media`, `src/image-generation`, `src/realtime-voice`, `src/canvas-host`, `extensions/comfy` |
