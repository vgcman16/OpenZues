...
- Repo-wide OpenClaw parity is estimated at ~79.2% overall, with a reasonable
  band of ~77-82%.
...
- The runtime/CLI/doctor native-bridge family is estimated at ~99.9% after the
  ...
  account-id/configured-id subpath, and agent-media-payload helper slices.
...
- Next repo-wide queue head: continue broader plugin SDK helper/runtime surface
  ...
  agent-media-payload path, starting with the next source-backed SDK helper
  subpath.
