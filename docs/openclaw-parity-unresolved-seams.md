...
- Repo-wide OpenClaw parity is estimated at ~79.6% overall, with a reasonable
  band of ~77-82%.
...
- The runtime/CLI/doctor native-bridge family is estimated at ~99.9% after the
  ...
  agent-config-primitives, ACP binding resolve, Anthropic CLI facade, and
  Anthropic Vertex auth-presence helper slices.
...
- Next repo-wide queue head: continue broader plugin SDK helper/runtime surface
  ...
  anthropic-cli/anthropic-vertex-auth-presence path, starting with the next
