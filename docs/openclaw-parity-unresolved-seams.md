...
- Repo-wide OpenClaw parity is estimated at ~77.9% overall, with a reasonable
  band of ~75-80%.
...
- The runtime/CLI/doctor native-bridge family is estimated at ~99.9% after the
  ...
  provider-auth-login, provider-auth facade, provider web-search contract, and
  provider web facade helper slices.
...
- Next repo-wide queue head: continue broader plugin SDK helper/runtime surface
  ...
  facade/provider-web-search-contract/provider-web facade path, starting with
  the next
