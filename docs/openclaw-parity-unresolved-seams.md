...
- Repo-wide OpenClaw parity is estimated at ~79.3% overall, with a reasonable
  band of ~77-82%.
...
- The runtime/CLI/doctor native-bridge family is estimated at ~99.9% after the
  ...
  account-id/configured-id subpath, agent-media-payload, and
  agent-config-primitives helper slices.
...
- Next repo-wide queue head: continue broader plugin SDK helper/runtime surface
  ...
  agent-media-payload/agent-config-primitives path, starting with the next
