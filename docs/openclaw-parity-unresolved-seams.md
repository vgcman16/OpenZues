...
- Repo-wide OpenClaw parity is estimated at ~77.7% overall, with a reasonable
  band of ~75-80%.
...
- The runtime/CLI/doctor native-bridge family is estimated at ~99.9% after the
  ...
  provider-auth-login, and provider-auth facade helper slices.
...
- Next repo-wide queue head: continue broader plugin SDK helper/runtime surface
  ...
  facade path, starting with the next source-backed SDK helper subpath.
