...
- The runtime/CLI/doctor native-bridge family is estimated at ~99.96% after the
  ...
  memory-core-host-query helper, memory-core-host-multimodal helper,
...
- The CLI/operator control-plane family is estimated at ~99.96% after the bundle
  ...
  gateway-backed remote secret resolution, and packaging surfaces.
- Fully locked bounded slices are now tracked in
  ...
  CLI/runtime breadth, packaging/doctor surfaces, QR remote gateway-backed
  secret resolution, and companion app parity.
...
inspect/doctor proof, `ruff check`, and `mypy`; checkpoint in `c11085d1`.
Plugin inspect human capability sections now mirror OpenClaw's
`src/cli/plugins-inspect-command.ts` capability output: human inspect renders
...
Plugin inspect human runtime surface sections now mirror OpenClaw's
`src/cli/plugins-inspect-command.ts` runtime surface output: human inspect
...
in `5ac316c1`.
Plugin inspect human MCP/LSP server sections now mirror OpenClaw's
`src/cli/plugins-inspect-command.ts` server output: human inspect renders
`MCP servers` and `LSP servers` sections from the native inspect payload.
...
inspect proof, `ruff check`, and `mypy`; checkpoint in `e0af8199`.
Plugin inspect human diagnostics section now mirrors OpenClaw's
`src/cli/plugins-inspect-command.ts` diagnostics output: human inspect renders
...
Plugin inspect human install section now mirrors OpenClaw's
`src/cli/plugins-inspect-command.ts` install output: human inspect renders
saved install records in OpenClaw field order, including source, paths,
recorded version, ClawHub/ClawPack metadata, size, and installed-at fields
...
Plugin inspect human header/bundle-format labels now mirror OpenClaw's
`src/cli/plugins-inspect-command.ts` header output: human inspect renders
capitalized `Status`, `Format`, `Source`, and `Shape` labels and includes
`Bundle format` when present. Verified on 2026-05-02 with the focused Claude
bundle inspect CLI test, adjacent plugin inspect/doctor proof, `ruff check`,
and `mypy`; checkpoint in `df4d586c`.
Plugin list verbose activation/import state now mirrors OpenClaw's
`src/cli/plugins-list-format.test.ts` formatting contract: human
`plugins list --verbose` renders `activated`, `imported`, `explicitly enabled`,
`activation source`, and sanitized `activation reason` rows from native plugin
...
`ruff check`, and `mypy`; checkpoint in `bc362484`.
Plugin list human enabled counts now mirror OpenClaw's
`src/cli/plugins-list-command.ts` header contract: human `plugins list
--verbose` renders `Plugins (enabled/total enabled)` instead of `loaded`,
using explicit record `enabled` values when present and falling back to
...
in `54bf33aa`.
Errored runtime-imported plugin projection now mirrors OpenClaw's
`src/plugins/status.test.ts` diagnostics-loaded import contract: runtime
inspect/diagnostics marks non-bundle plugin rows as `imported=true` when the
...
updated/unchanged outcome metadata. Marketplace entries whose source is a
separate `github`, `git`, or `git-subdir` repo now clone through the same
fakeable adapter, resolve the configured subpath inside the plugin clone, copy
the plugin into the durable data-dir install root, and clean up the clone.
...
Remaining plugin CLI parity is package/npm/clawhub install/update behavior and
...
- Closed the plugin inspect human base metadata seam from OpenClaw
  ...
  CLI test, adjacent plugin inspect/doctor proof, `ruff check`, and `mypy`;
  checkpoint in `c11085d1`.
- Closed the plugin inspect human capability sections seam from OpenClaw
  `src/cli/plugins-inspect-command.ts`: human inspect output now renders bundle
  capabilities and capability rows from the native inspect payload, including
  registered/inventory capability ids. Repo-wide parity is now estimated at
  ...
  adjacent plugin inspect/doctor proof, `ruff check`, and `mypy`; checkpoint
  in `2b161d5a`.
- Closed the plugin inspect human runtime surface sections seam from OpenClaw
  `src/cli/plugins-inspect-command.ts`: human inspect output now renders
  `Commands`, `CLI commands`, `Services`, and `Gateway methods` sections from
...
- Closed the plugin inspect human tools section seam from OpenClaw
  ...
  `ruff check`, and `mypy`; checkpoint in `5ac316c1`.
- Closed the plugin inspect human MCP/LSP server sections seam from OpenClaw
  `src/cli/plugins-inspect-command.ts`: human inspect output now renders
  `MCP servers` and `LSP servers` sections from the native inspect payload.
  Repo-wide parity is now estimated at ~55.4%; runtime/CLI/doctor and
  CLI/operator-control bounded paths remain ~99.9%. Verified with the focused
...
- Closed the plugin inspect human policy section seam from OpenClaw
  ...
  plugin inspect proof, `ruff check`, and `mypy`; checkpoint in `e0af8199`.
- Closed the plugin inspect human diagnostics section seam from OpenClaw
  `src/cli/plugins-inspect-command.ts`: human inspect output now renders
  ...
  adjacent plugin inspect proof, `ruff check`, and `mypy`; checkpoint in
  `667182c7`.
- Closed the plugin inspect human install section seam from OpenClaw
  `src/cli/plugins-inspect-command.ts`: human inspect output now renders
  `Install` rows for saved install records in OpenClaw field order. Repo-wide
  parity is now estimated at ~55.8%; runtime/CLI/doctor and
  CLI/operator-control bounded paths remain ~99.9%. Verified with the focused
...
- Closed the plugin inspect human header/bundle-format labels seam from
  OpenClaw `src/cli/plugins-inspect-command.ts`: human inspect output now
  renders capitalized `Status`, `Format`, `Source`, and `Shape` labels plus
  `Bundle format` for bundle plugins. Repo-wide parity is now estimated at
  ~56.1%; runtime/CLI/doctor and CLI/operator-control bounded paths remain
  ~99.9%. Verified with the focused Claude bundle inspect CLI test, adjacent
  plugin inspect/doctor proof, `ruff check`, and `mypy`; checkpoint in
  `df4d586c`.
- Closed the plugin list verbose activation/import state seam from OpenClaw
  `src/cli/plugins-list-format.test.ts`: human `plugins list --verbose` now
  renders `activated`, `imported`, `explicitly enabled`, `activation source`,
  and sanitized `activation reason` rows from native plugin records. Repo-wide
  parity is now estimated at ~56.2%; runtime/CLI/doctor and
  CLI/operator-control bounded paths remain ~99.9%. Verified with the focused
...
- Closed the plugin list human enabled-label seam from OpenClaw
  ...
  list/runtime proof, `ruff check`, and `mypy`; checkpoint in `bc362484`.
- Closed the plugin list human enabled-count seam from OpenClaw
  `src/cli/plugins-list-command.ts`: human `plugins list --verbose` now
  renders `Plugins (enabled/total enabled)` instead of `loaded`, using
  explicit `enabled` fields when present and status fallback for native
  OpenZues records. Repo-wide parity is now estimated at ~56.4%;
  runtime/CLI/doctor and CLI/operator-control bounded paths remain ~99.9%.
  Verified with the focused runtime-inventory CLI test, adjacent plugin
...
- Closed the errored runtime-imported plugin projection seam from OpenClaw
  `src/plugins/status.test.ts`: runtime inspect/diagnostics now marks
  non-bundle plugins as `imported=true` when the module was evaluated even if
  the final status is `error`. Repo-wide parity is now estimated at ~56.6%;
  runtime/CLI/doctor and CLI/operator-control bounded paths remain ~99.9%.
...
- Closed the configured-channel plugin owner activation projection seam from
  ...
  in `ae5c3986`.
- Closed the configured-channel disabled-owner policy seam from OpenClaw
  `src/plugins/channel-presence-policy.ts`,
  `src/plugins/manifest-owner-policy.ts`, and
  `src/plugins/activation-context.ts`: explicitly disabled manifest owners now
  remain blocked as `plugin-disabled`, do not enter the scoped plugin id set,
  and do not emit temporary activation config. Repo-wide parity is now
  estimated at ~56.9%; runtime/CLI/doctor and CLI/operator-control bounded
...
- Closed the configured-channel config/global owner trust gate seam from
  ...
  doctor proof, `ruff check`, and `mypy`; checkpoint in `0e6ce093`.
- Closed the configured-channel workspace owner activation gate seam from
  OpenClaw `src/plugins/channel-presence-policy.ts` and
  `src/plugins/config-activation-shared.ts`: workspace-origin manifest channel
  owners now require explicit activation through `plugins.allow` or
  `plugins.entries.<id>.enabled=true` before scoped configured-channel
  activation, otherwise the owner is projected as `untrusted-plugin`.
  Repo-wide parity is now estimated at ~57.2%; runtime/CLI/doctor and
  CLI/operator-control bounded paths remain ~99.9%. Verified with the focused
  workspace-owner activation helper test, full activation helper suite,
  adjacent plugin doctor proof, `ruff check`, and `mypy`; checkpoint in
  `bb9ef28a`.
- Closed the manifest toolMetadata availability gate seam from OpenClaw
  `src/plugins/tools.optional.test.ts`, `src/plugins/tools.ts`, and
  `src/plugins/manifest-tool-availability.ts`: native plugin doctor/runtime
  activation posture now distinguishes unavailable manifest-owned tools from
  metadata-only missing executors by evaluating auth/config availability
  signals before counting executor gaps. Repo-wide parity is now estimated at
  ~57.3%; runtime/CLI/doctor and CLI/operator-control bounded paths remain
...
- Closed the installed plugin runtime activation adapter seam from OpenClaw
  ...
  proof, `ruff check`, and `mypy`; checkpoint in `26e55209`.
- Closed the installed plugin disabled activation gate seam from OpenClaw
  `src/plugins/loader.test.ts`, `src/plugins/config-state.ts`, and
  `src/plugins/runtime/runtime-registry-loader.ts`: disabled manifest/load-path
  plugins are now excluded before native installed-plugin activation adapter
  dispatch, do not count as missing runtime executor gaps, and remain
  `imported=false`. Repo-wide parity is now estimated at ~57.5%;
  runtime/CLI/doctor and CLI/operator-control bounded paths remain ~99.9%.
  Verified with the focused plugin doctor disabled activation adapter test,
  adjacent plugin runtime CLI proof, `ruff check`, and `mypy`; checkpoint in
  `457021d6`.
- Closed the installed plugin inspect runtime activation adapter tool
  projection seam from OpenClaw `src/cli/plugins-inspect-command.ts`,
  `src/plugins/status.ts`, and
  `src/plugins/runtime/runtime-registry-loader.ts`: `plugins inspect <id>
  --runtime --json` now resolves native installed-plugin activation adapter
  tools after target-scoped runtime inventory and reports runtime capability
  mode plus concrete tool rows for the inspected plugin. Repo-wide parity is
  now estimated at ~57.6%; runtime/CLI/doctor and CLI/operator-control bounded
  paths remain ~99.9%. Verified with the focused plugin inspect installed
  activation adapter test, adjacent plugin runtime CLI proof, `ruff check`, and
  `mypy`; checkpoint in `fb4fca1b`.
- Closed the installed plugin scoped runtime activation load-context seam from
  OpenClaw `src/plugins/runtime/load-context.ts`,
  `src/plugins/runtime/runtime-registry-loader.ts`, `src/plugins/status.ts`,
  and `src/cli/plugins-inspect-command.ts`: native installed-plugin activation
  adapters now receive `onlyPluginIds`, `workspaceDir`, and
  `activationSourceConfig` for scoped runtime loads. Repo-wide parity is now
  estimated at ~57.7%; runtime/CLI/doctor and CLI/operator-control bounded
...
- Closed the Microsoft Teams delegated OAuth setup bootstrap seam from
  ...
  `6f368d37`.
- Closed the Microsoft Teams delegated OAuth completion seam from OpenClaw
  `extensions/msteams/src/oauth.flow.ts` and
  `extensions/msteams/src/oauth.token.ts`: native OpenZues CLI setup completion
  now parses the full redirect URL, enforces state matching, exchanges the
  authorization code through Azure v2 using the upstream form payload
  (`authorization_code`, redirect URI, code verifier, scopes), requires a
  refresh token, subtracts the five-minute expiry buffer, persists delegated
  access/refresh tokens with scopes and user principal metadata, and omits
...
- Closed the Microsoft Teams pin message action seam from OpenClaw
  `extensions/msteams/src/actions.ts`,
  `extensions/msteams/src/graph-messages.ts`, and
  `extensions/msteams/src/graph-messages.actions.test.ts`: native OpenZues
  `message.action` dispatch now supports `channel="msteams"`,
  `action="pin"`, resolves `target` / `to` plus `messageId`, uses
  route-backed Graph auth with stored delegated-token preference when
  available, rejects channel pinning with the upstream Graph v1.0 unavailable
  error, POSTs `message@odata.bind` to
  `/chats/{chatId}/pinnedMessages`, and projects OpenClaw-shaped `{ok,
  channel, action, pinnedMessageId}` results. Repo-wide parity is now
...
- Closed the Microsoft Teams unpin message action seam from OpenClaw
  ...
  action/provider proof, `ruff check`, and `mypy`; source/test checkpointed in
  `dafcd607`.
- Closed the Microsoft Teams list-pins message action seam from OpenClaw
  `extensions/msteams/src/actions.ts`,
  `extensions/msteams/src/graph-messages.ts`, and
  `extensions/msteams/src/graph-messages.read.test.ts`: native OpenZues
  `message.action` dispatch now supports `channel="msteams"`,
  `action="list-pins"`, resolves route-backed Graph chat targets, uses
  route-backed Graph auth with stored delegated-token preference when
  available, rejects channel list-pins with the upstream Graph v1.0
  unavailable error, GETs
  `/chats/{chatId}/pinnedMessages?$expand=message`, follows bounded
  `@odata.nextLink` pagination, and projects OpenClaw-shaped `{ok, channel,
...
- Closed the Microsoft Teams member-info action seam from OpenClaw
  `extensions/msteams/src/actions.ts`,
  `extensions/msteams/src/graph-members.ts`, and
  `extensions/msteams/src/graph-members.test.ts`: native OpenZues
  `message.action` dispatch now supports `channel="msteams"`,
  `action="member-info"`, trims `userId`, uses route-backed Graph auth with
  stored delegated-token preference when available, fetches `/users/{id}` with
  the upstream `$select` field set, and projects OpenClaw-shaped `{ok,
  channel, action, user}` results. Repo-wide parity is now estimated at
  ~67.5%. Verified with focused runtime pytest, adjacent Teams action/provider
  proof, `ruff check`, and `mypy`; source/test checkpointed in `8aa5f0a6`.
- Closed the Microsoft Teams channel-list action seam from OpenClaw
  `extensions/msteams/src/actions.ts`,
  `extensions/msteams/src/graph-teams.ts`, and
  `extensions/msteams/src/graph-teams.test.ts`: native OpenZues
  `message.action` dispatch now supports `channel="msteams"`,
  `action="channel-list"`, trims `teamId`, uses route-backed Graph auth with
  stored delegated-token preference when available, fetches
  `/teams/{teamId}/channels` with the upstream `$select` field set, follows
  bounded `@odata.nextLink` pagination, and projects OpenClaw-shaped `{ok,
  channel, action, channels, truncated}` results. Repo-wide parity is now
...
- Closed the Microsoft Teams edit message action seam from OpenClaw
  ...
  dispatch now supports `channel="msteams"`, `action="edit"`, resolves Teams
  conversation targets, accepts `text`/`content`/`message` content fallback
  plus `messageId`, uses route-backed Bot Framework auth, PUTs a message
  activity update to `/activities/{messageId}`, and projects OpenClaw-shaped
  `{ok, channel, conversationId}` results. Repo-wide parity is now estimated
...
- Closed the Microsoft Teams adaptive-card send action seam from OpenClaw
  `extensions/msteams/src/actions.ts`, `extensions/msteams/src/send.ts`, and
  `extensions/msteams/src/channel.actions.test.ts`: native OpenZues
  `message.action` dispatch now supports `channel="msteams"`,
  `action="send"` with a `card` payload, resolves Teams conversation targets,
  posts an Adaptive Card Bot Framework activity through the native route, and
  projects OpenClaw-shaped `{ok, channel, messageId, conversationId}`
  results. Repo-wide parity is now estimated at ~68.1%. Verified with focused
  runtime pytest, adjacent Teams action/provider proof, `ruff check`, and
  `mypy`; source/test checkpointed in `30fbcc69`.
- Closed the Twitch send message action seam from OpenClaw
  `extensions/twitch/src/actions.ts`, `extensions/twitch/src/outbound.ts`,
  and `extensions/twitch/src/actions.test.ts`: native OpenZues
  `message.action` dispatch now supports `channel="twitch"`,
  `action="send"`, accepts required `message` plus optional `to` scalar
  params, falls back to the route default channel, reuses the native
  route-backed Twitch chat sender and markdown stripping path, and projects
  OpenClaw-shaped `{ok, channel, messageId, timestamp}` results. Repo-wide
...
- Closed the imported OpenClaw plugin runtime tool factory context seam from
  `src/plugins/tool-types.ts`, `src/plugins/registry.ts`,
  `src/plugins/tools.ts`, and `src/gateway/tools-invoke-shared.ts`:
  `api.registerTool(factory, { name })` entries are now preserved by the
  native runtime importer, resolved at `tools.invoke` time, and passed a
  trusted OpenClaw-shaped tool context with config/runtimeConfig, workspace,
  agent/session identity, sender ownership, and delivery route metadata.
  Repo-wide parity is now estimated at ~70.7%. Verified with focused runtime
  factory-context pytest, adjacent plugin invoke proof, `ruff check`, and
  `mypy`; source/test checkpointed in `ef254cbf`.
- Closed the imported OpenClaw plugin SDK text-runtime helper seam from
  `src/plugin-sdk/text-runtime.ts` and `src/shared/string-coerce.ts`:
  native runtime entries can now import common string-coerce helpers including
  `normalizeOptionalString`, `normalizeNullableString`,
  `normalizeStringifiedOptionalString`, and `hasNonEmptyString` from
  `openclaw/plugin-sdk/text-runtime` and execute them through `tools.invoke`.
  Repo-wide parity is now estimated at ~70.8%. Verified with focused
  text-runtime helper pytest, adjacent plugin invoke proof, `ruff check`, and
  `mypy`; source/test checkpointed in `91918c38`.
- Closed the imported OpenClaw plugin SDK error-runtime helper seam from
  `src/plugin-sdk/error-runtime.ts` and `src/infra/errors.ts`: native runtime
  entries can now import `formatErrorMessage`, `formatUncaughtError`,
  `extractErrorCode`, `readErrorName`, and `collectErrorGraphCandidates` from
  `openclaw/plugin-sdk/error-runtime` and execute them through
...
- Closed the imported OpenClaw plugin SDK secret-input helper seam from
  `src/plugin-sdk/secret-input.ts` and `src/config/types.secrets.ts`: native
  runtime entries can now import literal-secret normalization, SecretRef
  coercion, inspect-mode resolution, and configured-secret detection helpers
  from `openclaw/plugin-sdk/secret-input` and execute them through
...
- Closed the imported OpenClaw plugin SDK reply-chunking helper seam from
  `src/plugin-sdk/reply-chunking.ts`, `src/auto-reply/chunk.ts`, and
  `src/auto-reply/tokens.ts`: native runtime entries can now import
  length/newline text chunking, provider/account chunk limit and chunk mode
  resolution, and silent reply token helpers from
  `openclaw/plugin-sdk/reply-chunking` and execute them through
...
- Closed the imported OpenClaw plugin SDK account-helper seam from
  `src/plugin-sdk/account-helpers.ts`,
  `src/channels/plugins/account-helpers.ts`, and
  `src/channels/plugins/account-action-gate.ts`: native runtime entries can now
  import account list/default resolution, normalized account lookup, merged
  account config projection, account/webhook snapshots, and account action
  gates from `openclaw/plugin-sdk/account-helpers` and execute them through
  `tools.invoke`. Repo-wide parity is now estimated at ~71.5%. Verified with
  focused account-helper pytest, adjacent plugin invoke proof, `ruff check`,
  and `mypy`; source/test checkpointed in `6d2cf33b`.
- Closed the imported OpenClaw plugin SDK account-core/account-resolution seam
  from `src/plugin-sdk/account-core.ts`, `src/plugin-sdk/account-resolution.ts`,
  `src/plugin-sdk/account-resolution-runtime.ts`, `src/channels/chat-type.ts`,
  `src/plugin-sdk/account-configured-ids.ts`, and `src/utils.ts`: native
  runtime entries can now import account-core reexports, configured id
  listing, default-account credential fallback, chat-type normalization, E.164
  normalization, home-relative path resolution, and path existence checks from
  `openclaw/plugin-sdk/account-core` / `account-resolution` and execute them
...
- Closed the imported OpenClaw plugin SDK boolean-param seam from
  `src/plugin-sdk/boolean-param.ts`: native runtime entries can now import the
  loose boolean tool parameter reader from `openclaw/plugin-sdk/boolean-param`
...
- Closed the imported OpenClaw plugin SDK direct-DM access seam from
  `src/plugin-sdk/direct-dm-access.ts`: native runtime entries can now import
  pairing-store direct-DM allowlist reads, access-group expansion for
  configured/store allowlists, open-DM allowlist blocking, resolved direct-DM
  access projection, command authorization runtime delegation, sender
  command-allow checks, pairing challenge callbacks, and blocked-sender
  callbacks while preserving generic SDK fallback behavior through
  `tools.invoke`. Repo-wide parity is now estimated at ~75.7%. Verified with
  focused direct-DM access pytest, adjacent plugin invoke proof, `ruff check`,
  and `mypy`; source/test checkpointed in `5258f537`.
- Closed the imported OpenClaw plugin SDK direct-DM guard-policy seam from
  `src/plugin-sdk/direct-dm-guard-policy.ts`: native runtime entries can now
  import default and overrideable pre-crypto guard policy construction, allowed
  kind overrides, future-skew/ciphertext/plaintext byte limits, nested
  rate-limit default merging, generic SDK fallback behavior, and the
  `channel-inbound` re-export through `tools.invoke`. Repo-wide parity is now
  estimated at ~75.8%. Verified with focused direct-DM guard-policy pytest,
  adjacent plugin invoke proof, `ruff check`, and `mypy`; source/test
  checkpointed in `02803c84`.
- Closed the imported OpenClaw plugin SDK direct-DM seam from
  `src/plugin-sdk/direct-dm.ts`, `src/plugin-sdk/inbound-envelope.ts`, and
  `src/plugin-sdk/inbound-reply-dispatch.ts`: native runtime entries can now
  import the direct-DM access and guard-policy re-exports plus the
  `dispatchInboundDirectDmWithRuntime` route/envelope/session-record/reply
  dispatch helper, with generic SDK fallback behavior and the `channel-inbound`
  re-export through `tools.invoke`. Repo-wide parity is now estimated at
  ~75.9%. Verified with focused direct-DM pytest, adjacent plugin invoke
  proof, `ruff check`, and `mypy`; source/test checkpointed in `6534f0db`.
- Closed the imported OpenClaw plugin SDK channel-send-result seam from
  `src/plugin-sdk/channel-send-result.ts`: native runtime entries can now
  import channel result stamping, batch stamping, empty outbound result
  construction, raw send result normalization, raw error string projection,
  attached text/media/poll adapter wrapping, and raw text/media adapter
  wrapping while preserving generic SDK fallback behavior through
  `tools.invoke`. Repo-wide parity is now estimated at ~76.0%. Verified with
  focused channel-send-result pytest, adjacent plugin invoke proof,
  `ruff check`, and `mypy`; source/test checkpointed in `c73fb961`.
- Closed the imported OpenClaw plugin SDK channel-pairing seam from
  `src/plugin-sdk/channel-pairing.ts`, `src/plugin-sdk/pairing-access.ts`,
  `src/channels/plugins/pairing-adapters.ts`,
  `src/pairing/pairing-challenge.ts`, and `src/pairing/pairing-store.ts`:
  native runtime entries can now import scoped channel pairing controllers,
  channel-bound challenge issuers, pairing reply delivery, prefix-stripping
  allowFrom adapters, text pairing adapters, logged approval notifiers,
  allowFrom store path resolution, and allowFrom store readers while
  preserving generic SDK fallback behavior through `tools.invoke`. Repo-wide
  parity is now estimated at ~76.1%. Verified with focused channel-pairing
  pytest, adjacent plugin invoke proof, `ruff check`, and `mypy`; source/test
  checkpointed in `f0a21c8c`.
- Closed the imported OpenClaw plugin SDK command-auth seam from
  `src/plugin-sdk/command-auth.ts`, `src/plugin-sdk/access-groups.ts`,
  `src/plugin-sdk/direct-dm.ts`, `src/auto-reply/command-detection.ts`,
  `src/auto-reply/commands-registry.ts`, and
  `src/channels/command-gating.ts`: native runtime entries can now import
  sender command authorization across DM/group/pairing-store/access-group
  sources, runtime-backed authorization wrappers, direct-DM authorization
  outcomes, command-gating helpers, command detection helpers, direct-DM and
  access-group compatibility re-exports, and deprecated command-status
  builders while preserving generic SDK fallback behavior through
...
- Closed the imported OpenClaw plugin SDK allowlist-config-edit seam from
  `src/plugin-sdk/allowlist-config-edit.ts`: native runtime entries can now
  import DM/group and legacy-DM config path resolution, configured entry
  coercion, flat/nested override collectors and account-scoped resolvers,
  token-gated name resolution, account-scoped config editor target selection,
  add/remove/duplicate edit semantics, top-level/default-account writes, legacy
  `dm.allowFrom` cleanup, and generic SDK re-exports through `tools.invoke`.
...
- Closed the imported OpenClaw plugin SDK group-access seam from
  ...
  plugin invoke proof, `ruff check`, and `mypy`; source/test checkpointed in
  `88510816`.
- Closed the imported OpenClaw plugin SDK provider-selection-runtime seam from
  `src/plugin-sdk/provider-selection-runtime.ts`: native runtime entries can
  now import explicit provider selection, missing explicit provider reporting,
  auto-select ordering, canonical plus selected raw config merging,
  configured-capability provider resolution, and no-provider/
  provider-not-configured failure codes through `tools.invoke`. Repo-wide
...
- Closed the imported OpenClaw plugin SDK simple-completion-runtime
  `extractAssistantText` helper seam from
  `src/plugin-sdk/simple-completion-runtime.ts` and
  `src/agents/pi-embedded-utils.ts`: native runtime entries can require
  scoped/unscoped simple-completion runtime helpers and receive deterministic
  assistant text extraction for string content, text blocks, non-text
  filtering, leaked tool XML stripping, MiniMax invocation stripping, and
  HTTP-style assistant error copy. Repo-wide parity is now estimated at
  ~84.6%. Verified with focused simple-completion-runtime pytest, adjacent
  runtime/provider proof, adjacent imported-plugin proof, `ruff check`, and
  `mypy`; source/test checkpointed in `13a0fa6d`. Broader completion
  model/auth transport helpers remain open.
- Closed the imported OpenClaw plugin SDK approval-reply-runtime helper seam
  from `src/plugin-sdk/approval-reply-runtime.ts`,
  `src/infra/exec-approval-reply.ts`,
  `src/infra/exec-approval-command-display.ts`,
  `src/infra/exec-approvals.ts`, and
  `src/plugin-sdk/approval-renderers.ts`: native runtime entries can require
  scoped/unscoped approval reply helpers, build action descriptors and
  interactive button payloads, parse `/approve` commands, resolve allowed
  decisions, project command display text, build exec/plugin pending reply
  payloads, and extract approval metadata. Repo-wide parity is now estimated at
  ~84.7%. Verified with focused approval-reply-runtime pytest, adjacent
  approval/runtime proof, adjacent imported-plugin proof, `ruff check`, and
  `mypy`; source/test checkpointed in `2215e898`. Broader approval gateway,
  client, delivery, native, and handler runtimes remain open.
- Closed the imported OpenClaw plugin SDK approval-client-helpers seam from
  `src/plugin-sdk/approval-client-helpers.ts` and
  `src/infra/approval-request-filters.ts`: native runtime entries can require
  scoped/unscoped approval client helpers, resolve channel approval enablement,
  match configured approval targets by channel/account/sender, apply agent and
  session filters with session-key agent fallback, compose channel approval
  profiles, and suppress local prompts from approval metadata. Repo-wide
  parity is now estimated at ~84.8%. Verified with focused
  approval-client-helpers pytest, adjacent approval proof, adjacent
  imported-plugin proof, `ruff check`, and `mypy`; source/test checkpointed in
  `8a40507d`. Broader approval gateway, delivery, native, and handler runtimes
  remain open.
- Closed the imported OpenClaw plugin SDK approval-delivery-helpers seam from
  `src/plugin-sdk/approval-delivery-helpers.ts` and
  `src/plugin-sdk/approval-delivery-runtime.ts`: native runtime entries can
  require scoped/unscoped approval delivery helpers, create and split channel
  approval capabilities, preserve the deprecated `approvals` surface alias,
  authorize exec/plugin approvals by sender, report native DM/channel delivery
  availability, and suppress forwarding fallback only for matching native
  delivery surfaces. Repo-wide parity is now estimated at ~84.9%. Verified
  with focused approval-delivery-helpers pytest, adjacent approval proof,
  adjacent imported-plugin proof, `ruff check`, and `mypy`; source/test
...
- Closed the imported OpenClaw plugin SDK approval-handler-adapter-runtime seam
  from `src/plugin-sdk/approval-handler-adapter-runtime.ts` and
  `src/infra/approval-handler-adapter-runtime.ts`: native runtime entries can
  require scoped/unscoped approval handler adapter helpers, preserve the
  `approval.native` context capability, expose eager availability checks,
  lazy-load the backing native runtime once, delegate presentation, transport,
  and interaction hooks, and fire observe hooks only after a runtime has been
  loaded. Repo-wide parity is now estimated at ~85.2%. Verified with focused
  approval-handler-adapter-runtime pytest, adjacent approval proof, adjacent
  imported-plugin proof, `ruff check`, and `mypy`; source/test checkpointed in
  `21aa746b`. The broader approval handler runtime/gateway lifecycle remains
  open.
- Closed the imported OpenClaw plugin SDK approval-handler-runtime
  adapter-factory seam from `src/plugin-sdk/approval-handler-runtime.ts` and
  `src/infra/approval-handler-runtime.ts`: native runtime entries can require
  scoped/unscoped approval handler runtime helpers, preserve the
  `approval.native` context capability, wrap native runtime specs into
  canonical availability, presentation, transport, interaction, and observe
  adapters, preserve `eventKinds` and custom approval-kind resolution, and keep
  fallback-safe access for broader handler functions that remain open.
  Repo-wide parity is now estimated at ~85.3%. Verified with focused
  approval-handler-runtime pytest, adjacent approval proof, adjacent
  imported-plugin proof, `ruff check`, and `mypy`; source/test checkpointed in
  `3434972a`. The broader handler lifecycle/gateway integration remains open.
- Closed the imported OpenClaw plugin SDK approval-runtime aggregate seam from
  `src/plugin-sdk/approval-runtime.ts`: native runtime entries can require
  scoped/unscoped approval runtime helpers, resolve exec approval decisions,
  build exec/plugin pending payloads, extract approval metadata, compose
  approver authorization, channel approval profiles, delivery capabilities,
  native origin targets, and request filters through the aggregate barrel.
  Repo-wide parity is now estimated at ~85.4%. Verified with focused
  approval-runtime pytest, adjacent approval proof, adjacent imported-plugin
  proof, `ruff check`, and `mypy`; source/test checkpointed in `63554e0a`.
  The broader approval gateway/native handler lifecycle remains open.
- Closed the imported OpenClaw plugin SDK approval-gateway-runtime resolver
  seam from `src/plugin-sdk/approval-gateway-runtime.ts` and
  `src/infra/approval-gateway-resolver.ts`: native runtime entries can require
  scoped/unscoped approval gateway helpers, route plugin approvals through
  `plugin.approval.resolve`, route exec approvals through
  `exec.approval.resolve`, preserve gateway URL/config/display-name options,
  and fall back to plugin approval resolution only for not-found exec approval
  errors when explicitly enabled. Repo-wide parity is now estimated at
  ~85.5%. Verified with focused approval-gateway-runtime pytest, adjacent
  approval proof, adjacent imported-plugin proof, `ruff check`, and `mypy`;
  source/test checkpointed in `29c62d3f`. The broader native approval runtime
  and handler lifecycle remains open.
- Closed the imported OpenClaw plugin SDK
  `createChannelNativeApprovalRuntime` factory seam from
  ...
  aggregate `approval-runtime` barrel. Repo-wide parity is now estimated at
  ~85.6%. Verified with focused approval-native-runtime factory pytest,
  adjacent approval proof, adjacent imported-plugin proof, `ruff check`, and
  `mypy`; source/test checkpointed in `67a72452`. Expiration scheduling and the
  higher-level capability handler wrapper remain open.
- Closed the imported OpenClaw plugin SDK `createChannelApprovalHandler`
  wrapper seam from `src/infra/approval-handler-runtime.ts`: native runtime
  entries can map the upstream `{ runtime, content, transport, lifecycle }`
  handler adapter shape onto the native approval runtime factory, preserving
  approval kind, pending content, delivery callbacks, active entries, and
  resolved finalization. Repo-wide parity is now estimated at ~85.7%.
  Verified with focused approval-handler-runtime wrapper pytest, adjacent
  approval proof, adjacent imported-plugin proof, `ruff check`, and `mypy`;
  source/test checkpointed in `e23b4e9e`. The higher-level capability handler
  wrapper remains open.
- Closed the imported OpenClaw plugin SDK
  `createChannelApprovalHandlerFromCapability` seam from
  `src/infra/approval-handler-runtime.ts`: native runtime entries can turn a
  channel approval capability's native adapter/runtime into a handler, build
  pending/resolved/expired approval views, deliver and bind pending entries,
  call loaded observe hooks with unwrapped entries, unbind on resolution, apply
  final update/delete/clear-actions results, and return `null` when a
  capability has no native runtime. Repo-wide parity is now estimated at
  ~85.8%. Verified with focused approval-handler capability pytest, adjacent
  approval proof, adjacent imported-plugin proof, `ruff check`, and `mypy`;
  source/test checkpointed in `6ea77847`. Native expiration scheduling remains
  open.
- Closed the imported OpenClaw plugin SDK native approval expiration
  scheduling seam from `src/infra/approval-native-runtime.ts`: requested
  approvals with `expiresAtMs` install a timer, resolved/expired/stop paths
  clear timers, and timer expiry finalizes the stored active entries through
  the same expired lifecycle path. Repo-wide parity is now estimated at
...
- Closed the imported OpenClaw plugin SDK entrypoints seam from
  `src/plugin-sdk/entrypoints.ts` and
  `scripts/lib/plugin-sdk-entrypoints.json`: scoped and unscoped native runtime
  entries expose the canonical SDK entrypoint/subpath arrays, facade and
  public plugin-owned entrypoint lists, source/specifier/export-map builders,
  and expected dist artifact listing. Repo-wide parity is now estimated at
  ~88.2%. Verified with focused entrypoints pytest, adjacent SDK helper proof,
  adjacent imported-plugin proof, `ruff check`, and `mypy`; source/test
  checkpointed in `bd810f14`.
- Closed the imported OpenClaw plugin SDK diffs seam from
  `src/plugin-sdk/diffs.ts`: scoped and unscoped native runtime entries expose
  the narrow bundled-diffs helper surface for `definePluginEntry` plus
  `resolvePreferredOpenClawTmpDir`, and `definePluginEntry` now uses
  OpenClaw-shaped cached lazy config-schema resolution with the empty config
  schema default. Repo-wide parity is now estimated at ~88.3%. Verified with
  focused diffs pytest, adjacent SDK helper proof, adjacent imported-plugin
  proof, `ruff check`, and `mypy`; source/test checkpointed in `008c6120`.
- Closed the imported OpenClaw plugin SDK ACPX seam from
  `src/plugin-sdk/acpx.ts`, `src/acp/runtime/errors.ts`,
  `src/acp/runtime/registry.ts`, `src/plugin-sdk/windows-spawn.ts`, and
  `src/secrets/provider-env-vars.ts`: scoped and unscoped native runtime
  entries expose `AcpRuntimeError`, ACP backend registration/unregistration,
  Windows spawn materialization/policy helpers, and provider-auth env filtering
  helpers. Repo-wide parity is now estimated at ~88.4%. Verified with focused
  ACPX pytest, adjacent SDK helper proof, adjacent imported-plugin proof,
  `ruff check`, and `mypy`; source/test checkpointed in `c7541c95`.
- Closed the imported OpenClaw plugin SDK ACP runtime backend seam from
  `src/plugin-sdk/acp-runtime-backend.ts`, `src/acp/runtime/errors.ts`, and
  `src/acp/runtime/registry.ts`: scoped and unscoped native runtime entries
  expose ACP runtime error checks, backend lookup/require/registration/removal,
  and the lightweight reply-hook early-return path, with an honest unavailable
  result when full ACP dispatch would be required by the native bridge.
...
- Closed the imported OpenClaw plugin SDK ACP binding runtime seam from
  `src/plugin-sdk/acp-binding-runtime.ts`,
  `src/acp/persistent-bindings.lifecycle.ts`, and
  `src/acp/persistent-bindings.resolve.ts`: scoped and unscoped native runtime
  entries expose configured ACP binding resolution plus readiness ensure
  semantics through the injected ACP session manager, including already-ready,
  initialize, and no-binding paths. Repo-wide parity is now estimated at
  ~88.7%. Verified with focused ACP binding pytest, adjacent SDK helper proof,
  adjacent imported-plugin proof, `ruff check`, and `mypy`; source/test
  checkpointed in `37b428c1`.
- Closed the imported OpenClaw plugin SDK CLI runtime seam from
  `src/plugin-sdk/cli-runtime.ts`, `src/cli/command-format.ts`,
  `src/cli/parse-duration.ts`, `src/cli/command-options.ts`,
  `src/cli/argv-invocation.ts`, `src/cli/help-format.ts`,
  `src/cli/cli-utils.ts`, `src/cli/command-registration-policy.ts`,
  `src/cli/program/register-command-groups.ts`, `src/cli/wait.ts`,
  `src/terminal/note.ts`, `src/terminal/prompt-style.ts`, and
  `src/terminal/theme.ts`: scoped and unscoped native runtime entries expose
  CLI command formatting, duration parsing, parent-option inheritance,
  command-group registration, runtime error handling, argv projection,
  lazy-subcommand policy, terminal note/theme helpers, and version metadata.
  Repo-wide parity is now estimated at ~88.8%. Verified with focused CLI
  runtime pytest, adjacent SDK helper proof, adjacent imported-plugin proof,
...
- Closed the imported OpenClaw plugin SDK provider setup seam from
  `src/plugin-sdk/provider-setup.ts`,
  `src/plugin-sdk/self-hosted-provider-setup.ts`,
  `src/plugins/provider-self-hosted-setup.ts`, and
  `src/agents/self-hosted-provider-defaults.ts`: scoped and unscoped native
  runtime entries expose self-hosted default constants, default model patching,
  OpenAI-compatible local discovery test guards, provider discovery projection,
  interactive auth-result helpers, and non-interactive configuration with
  model/auth-profile/default model updates. Repo-wide parity is now estimated
  at ~89.0%. Verified with focused provider setup pytest, adjacent SDK helper
  proof, provider-auth API-key regression proof, adjacent imported-plugin
  proof, `ruff check`, `mypy`, and `git diff --check`; source/test
  checkpointed in `25ad92b6`.
- Closed the imported OpenClaw plugin SDK LM Studio seam from
  `src/plugin-sdk/lmstudio.ts`, `src/plugin-sdk/lmstudio-runtime.ts`,
  `extensions/lmstudio/src/models.ts`, and
  `extensions/lmstudio/src/runtime.ts`: scoped and unscoped native runtime
  entries expose LM Studio default constants, server/inference base URL
  normalization, provider config normalization, auth-header construction,
  loaded context-window resolution, reasoning capability/compat projection, and
  model wire entry mapping without leaking generic SDK exports. Repo-wide
  parity is now estimated at ~89.1%. Verified with focused LM Studio runtime
...
- Closed the imported OpenClaw plugin SDK runtime secret resolution seam from
  ...
  `ruff check`, `mypy`, and `git diff --check`; source/test checkpointed in
  `d9574b0f`.
- Closed the imported OpenClaw plugin SDK memory core host query seam from
  `src/plugin-sdk/memory-core-host-query.ts`,
  `packages/memory-host-sdk/src/query.ts`, and
  `packages/memory-host-sdk/src/host/query-expansion.ts`: scoped and unscoped
  native runtime entries expose `extractKeywords` and
  `isQueryStopWordToken`, including stop-word filtering, duplicate/numeric
  rejection, and CJK trigram-tokenizer behavior without generic SDK leakage.
  Repo-wide parity is now estimated at ~89.3%. Verified with focused
  memory-core-host-query pytest, adjacent SDK helper proof, adjacent
  imported-plugin proof, `ruff check`, `mypy`, and `git diff --check`;
  source/test checkpointed in `0eaf8bf8`.
- Closed the imported OpenClaw plugin SDK memory core host multimodal seam
  from `src/plugin-sdk/memory-core-host-multimodal.ts`,
  `packages/memory-host-sdk/src/multimodal.ts`, and
  `packages/memory-host-sdk/src/host/multimodal.ts`: scoped and unscoped
  native runtime entries expose `normalizeMemoryMultimodalSettings` and
  `isMemoryMultimodalEnabled`, including default image/audio modality
  selection, max-file-byte clamping/flooring, invalid modality filtering, and
  enabled-state checks. Repo-wide parity is now estimated at ~89.4%. Verified
  with focused memory-core-host-multimodal pytest, adjacent SDK helper proof,
  adjacent imported-plugin proof, `ruff check`, `mypy`, and `git diff --check`;
  source/test checkpointed in `5be944f7`.
- Closed the imported OpenClaw plugin SDK memory core host secret seam from
  `src/plugin-sdk/memory-core-host-secret.ts`,
  `packages/memory-host-sdk/src/secret.ts`,
  `packages/memory-host-sdk/src/host/secret-input.ts`, and
  `packages/memory-host-sdk/src/host/secret-input-utils.ts`: scoped and
  unscoped native runtime entries expose `hasConfiguredMemorySecretInput` and
  `resolveMemorySecretInputString`, including inline secret trimming,
  env-backed SecretRef resolution, legacy `secretref-env:` marker handling,
  and unresolved-ref errors. Repo-wide parity is now estimated at ~89.5%.
  Verified with focused memory-core-host-secret pytest, adjacent SDK helper
  proof, adjacent imported-plugin proof, `ruff check`, `mypy`, and
  `git diff --check`; source/test checkpointed in `78f2a009`.
- Closed the imported OpenClaw plugin SDK memory core host events seam from
  `src/plugin-sdk/memory-core-host-events.ts` and
  `src/memory-host-sdk/events.ts`: scoped and unscoped native runtime entries
  expose `MEMORY_HOST_EVENT_LOG_RELATIVE_PATH`,
  `resolveMemoryHostEventLogPath`, `appendMemoryHostEvent`, and
  `readMemoryHostEvents`, including JSONL append/read behavior, invalid-line
  skipping, limit handling, and missing-log empty results. Repo-wide parity is
  now estimated at ~89.6%. Verified with focused memory-core-host-events
  pytest, adjacent SDK helper proof, adjacent imported-plugin proof,
  `ruff check`, `mypy`, and `git diff --check`; source/test checkpointed in
  `d5695975`.
- Closed the imported OpenClaw plugin SDK memory core host status seam from
  `src/plugin-sdk/memory-core-host-status.ts`,
  `packages/memory-host-sdk/src/status.ts`, and
  `packages/memory-host-sdk/src/host/status-format.ts`: scoped and unscoped
  native runtime entries expose `resolveMemoryVectorState`,
  `resolveMemoryFtsState`, and `resolveMemoryCacheSummary`, including
  OpenClaw `tone` / `state` / `text` projections. Repo-wide parity is now
...
- Next repo-wide queue head: continue broader plugin SDK helper/runtime surface
  ...
  system-event-runtime/oauth-utils/runtime-config-snapshot/runtime-fetch/runtime-doctor/runtime-secret-resolution/memory-core-host-query/memory-core-host-multimodal/memory-core-host-secret/memory-core-host-events/memory-core-host-status/provider-setup/
...
- Current queue-head adjustment: imported plugin SDK `optional-channel-setup`
