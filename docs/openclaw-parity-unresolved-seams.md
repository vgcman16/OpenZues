...
`cron.update` now persists agentTurn payload extras (`thinking`,
`timeoutSeconds`, `lightContext`, and `toolsAllow`). Remaining cron gaps are
`cron.add` support for the same payload extras, CLI flags for those extras,
