...
| CLI + operator control plane | Partial | ~83% | Health, status JSON breadth flags, continue, queue, recover/harden, gateway doctor, delivery replay, route creation, direct route send/poll, sandbox inventory/explain/recreate, sessions spawn/wait, models list/status, plugins list/doctor/inspect/info/marketplace list/enable/disable, and operator monitor surfaces exist; broader runtime CLI/TUI breadth remains. |
