...
| Gateway session/tool contracts | Active | ~94% | `sessions_history`, `session_status`, `sessions_list`, `sessions_send`, `sessions_spawn`, `sessions_yield`, `sessions.create`, `sessions.patch`, `sessions.delete`, `tools.invoke` projections, visibility policy, configured and omitted subagent timeout defaults, completion-expectation metadata, lightweight bootstrap context, child task envelopes, lifecycle policy metadata, terminal cleanup consumption, and chat/session transcript contracts are now the live queue head; ACP harness spawning and deeper native executor/provider hooks remain. |
...
- ACP-backed `sessions.spawn` harness execution, sandboxed target runtimes, persistent thread-binding hooks, and completion-announcement orchestration beyond the bounded local cleanup implementation.
