...
| Channels + direct announce delivery | Strong partial | ~95% | Shared outbound runtime ownership spans direct send/poll, explicit announce, saved replays, native adapters, Slack/Telegram/Discord/WhatsApp routes, CLI route send/poll commands, channel status probe option metadata, channel capabilities reports, route-backed channel resolve, provider result metadata, OpenClaw-style send reply/thread/silent/document fields, Telegram native document/reply/silent/thread payloads, admin-scoped chat origin/system provenance, A2A announce/reply loops, and idle `sessions.steer` runtime sends; broader per-provider option coverage remains open. |
...
- Broader provider-native outbound runtime breadth for all provider-specific reply/silent/document/media edge cases beyond the route send/poll CLI surface, including live provider-backed channel resolve/probe adapters.
