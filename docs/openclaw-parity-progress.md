...
- Discord `message.action timeout` now covers OpenClaw's explicit-until
  moderation path by PATCHing the member endpoint with
  `communication_disabled_until` and returning the upstream
  `{ok: true, member}` payload.
- Verified the Discord timeout-action slice with `python -m pytest
  tests\test_ops_mesh.py -q -k "discord_timeout_route"` (`1 passed`),
  adjacent Discord action proof `python -m pytest tests\test_ops_mesh.py -q -k
  "message_action_dispatches_discord"` (`34 passed`), `ruff check
  src\openzues\services\ops_mesh.py tests\test_ops_mesh.py`, and `mypy
  src\openzues\services\ops_mesh.py`.
...
- Discord timeout duration aliases and moderation audit-log reason headers
  remain open after the explicit-until timeout slice.
