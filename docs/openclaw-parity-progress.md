...
| CLI + operator control plane | Partial | ~79% | Health, status, continue, queue, recover/harden, gateway doctor, delivery replay, route creation, direct route send/poll, sandbox inventory/explain/recreate, sessions spawn/wait, models list, plugins list/doctor/inspect/info, and operator monitor surfaces exist; broader runtime CLI/TUI breadth remains. |
