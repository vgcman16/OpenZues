...
- Broader provider-native outbound runtime breadth for remaining provider-specific media edge cases beyond the route send/poll CLI surface, including production live provider-backed channel resolve plus Discord/WhatsApp probe adapters.
...
- `python -m pytest tests\test_cli.py -q -k "route_backed_slack_probe or route_backed_telegram_probe or channels_status_json or channels_capabilities_json"`: 7 passed after rechecking channel probe CLI surfaces.
