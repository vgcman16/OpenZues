...
| Cron wake/delivery | Strong partial | ~97% | Direct send/poll, provider route callbacks, native route setup, replay/test dispatch, provider error/result metadata, OpenClaw-style cron-expression schedules, due-run behavior, session-key wake routing, retry/backoff, the CLI simple command group, and the first CLI add path are verified. |
